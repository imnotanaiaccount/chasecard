...
// Haptic buzzing is bundled in here rather than getting its own toggle:
// it's a physical motion stimulus in the same family as what this
// setting exists to let people turn down.
//
...
function vibrate(pattern){ if(reducedMotion() || !navigator.vibrate) return; try{navigator.vibrate(pattern);}catch(e){} }
...
async function getSetsYugioh(){
  ...
  try{
    ...
    const FIXED_CONTENT_PATTERN = /\b(structure deck|starter deck|deck kit|legendary decks?|collector'?s?\s*tins?|\btins?\b|box set)\b/i;
    const seen = new Set();
...
function packCompositionFor(game, packSize, setMeta){
  ...
  if(packSize === 11) return { commons: 5, uncommons: 3, hitSlots: 2, ygo: false };
...
function generatePack(cards, game, packSize, setMeta){
  ...
  const { commons: nCommons, uncommons: nUncommons, hitSlots: nHits, ygo, battlePack } = packCompositionFor(game, packSize, setMeta);
  ...
  for(let i=0;i<nHits;i++){
    ...
    // modern non-Yu-Gi-Oh pack only has ONE genuine chase-rarity roll.
    // The 2nd "hit slot" here used to roll the SAME big-rarity table a
    // second time, roughly doubling real-world odds of a Double Rare+.
    // It's now a plain reverse holo pull instead (still shown as foil,
    // no shot at Double Rare and up). Pokemon-specific (see
    // isModernPokemonTen above) -- One Piece's Premium Booster is also a
    // 10-card, non-ygo pack, but this correction and HIT_WEIGHTS_REAL are
    // both sourced from real Pokemon pull data and don't apply to it.
    if(isModernPokemonTen && i > 0){
...
async function renderProfile() {
  ...
  wrap.innerHTML = `
      ...
      <div style="margin-top:18px; padding-top:16px; border-top:1px solid var(--edge);">
        ...
        <p class="hint" style="margin:2px 0 10px;">Turns off confetti, screen shake, screen flash, the holo shimmer sweep, the legendary godray/banner overlays, holo tilt, and haptic buzzing during pack opening. The pack tear and card flip stay — this only calms the celebration on top. Follows your device's reduced-motion setting by default.</p>

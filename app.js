    ...
    try {
      if ('caches' in window) {
        ...
        if (!res) {
          ...
          // it flow through the exact same cache-then-fetch path (and
          // the same cache key) as every other image source.
...
async function getCardsForSet(setId){
  const key = 'cache_cards_v10_' + setId; // v3->v4: JP rarity; v4->v5: pokemontcg.io image fallback; v5->v6: fixed fallback to use English name; v6->v7: curated Trainer names; v7->v8: dexId-based fallback; v8->v9: empty-result handling fix; v9->v10: added PokéWallet native-JP-art fallback (top priority when matched)
  ...
  if (setId.startsWith('jp-')) {
    ...
    try{
      ...
      if (jaCards.some(c => !c.image && !enByLocalId[c.localId])) {
      ...
      for (const c of jaCards) {
        if (c.image || enByLocalId[c.localId]) continue;
      ...
      const data = jaCards.map(c => {
        ...
        const pwUrl = !img ? pokeWalletImgByCardId[c.id] : null;

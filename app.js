...
async function getCardsForSet(setId){
  ...
  if (setId.startsWith('jp-')) {
    ...
    try{
      ...
      const data = (setData.cards || []).map(c => ({
        id: 'jp-' + c.id,
        name: c.name,
        rarity: c.rarity || '',
        images: {
...
async function renderHome(){
  ...
  try{
    ...
    const tasks = displaySets.map(s => async () => {
      ...
      if (!imgEl) return;
      const FALLBACK_SRC = 'data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIxIiBoZWlnaHQ9IjEiPjwvc3ZnQSs+';
      try {

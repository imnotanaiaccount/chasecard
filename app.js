...
function resolveAndAppendSetCard(s, myGen, gridEl) {
  return (async () => {
    ...
    card.innerHTML = `<img src="" onerror="this.src='data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIxIiBoZWlnaHQ9IjEiPjwvc3ZnPg==';this.classList.add('loaded')" onload="this.classList.add('loaded')" alt=""/><div class="cost-badge">${costDisplay} cr</div><div class="name">${s.name}</div><div class="meta">${s.series}</div>`;
...
function openRevealScreen(setMeta, pack, bgUrl, openingId){
  ...
  screen.innerHTML = `
    <div class="reveal-header">
      ...
      <button class="close-x" id="sound-toggle" style="margin-right:6px;">${SFX.isMuted() ? '🔇' : '🔊'}</button>
  ...
  function boot(){
    ...
    const soundToggle = $('#sound-toggle', screen);

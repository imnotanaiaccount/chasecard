...
function putSharedSetCache(setId, data) {
  ...
  sb.from(SET_CARD_CACHE_TABLE)
    .upsert({ set_id: setId, data, updated_at: new Date().toISOString() })
    .then(() => {})
    .catch(() => {});
}
...
const ImgCache = {
  ...
  // lookup is keyed by the Supabase mirror URL (always a real https
  // endpoint) instead, while blobUrls (and everything callers pass
  // around) stays keyed by the original url.
  _cacheKeyFor(url) {
    return `${CONFIG.SUPABASE_URL}/storage/v1/object/public/${POKEWALLET_MIRROR_BUCKET}/${this._mirrorPath(url)}`;
  },
  async has(url) {
    if (!url) return false;

...
// Companion to syncAchievementsQuiet, for the 3 badges that aren't
// computed by that RPC's own SQL and instead get checked client-side
// (set_complete/collector/whale_watcher — see award_achievement's
// allow-list). award_achievement now returns whether it actually
// inserted a new row, so this can show the same celebration + credit
// refresh syncAchievementsQuiet gives every other badge, instead of the
// silent myAchievements-only update these three used to get.
...
function renderAllBadgesPanel(userProfile){
  ...
  const statValues = {
    ...
    sets_completed_count: Number(store.get(scopedKey('sets_completed_count')) || 0),
  };
...
function renderPrivacyPolicyHTML(){
  ...
  return `
    ...
    ${legalSection('Third-party services', `
      ...
      • <b>Affiliate networks</b> (eBay Partner Network, TCGplayer/Impact, GameStop/Rakuten) — power the "Buy on..." links; clicking one sends you to that retailer's site under their own privacy policy.<br>

      ...
      const swCode = `
        ...
        self.addEventListener('fetch', e => {
          ...
          if (isImage) {
            ...
            // BUGFIX 2026-09-13: this was cache-first for EVERY non-image
            // request — including app.js and index.html themselves. Once
            // this service worker was installed on a device even once, it
            // would keep serving whatever app.js it originally cached
            // FOREVER, no matter how many times the real file changed on
            // the server — a normal reload doesn't bypass a service
            // worker's fetch handler, so "redeploy the fix" and "reload
            // the page" both silently did nothing once this had ever run.
            // This is almost certainly why several fixes this session
            // appeared not to work at all. Switched to network-first: try
            // the real network every time, update the cache with whatever
            // comes back, and only fall back to the cached copy if the
            // network request itself fails (genuinely offline) — the
            // standard safe pattern for an app shell that's expected to
            // change, instead of one meant for static assets.
            e.respondWith(

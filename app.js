...
function setupRealtimeNotifications(){
  ...
  const duelCh = sb.channel(`duel-notify-${uid}`)
    ...
    .on('postgres_changes', { event:'UPDATE', schema:'public', table:'duels', filter:`challenger_id=eq.${uid}` }, (payload) => {
      ...
      if(status === 'pending_packs') toast('⚔️ Your duel challenge was accepted — open your pack!');
...
function describeActivityEvent(ev){
  ...
  switch(ev.type){
    case 'new_follower': return { icon:'👋', text:`${who} started following you.` };
    case 'trade_offer_received': return { icon:'🔄', text:`${who} sent you a trade offer.` };
    case 'trade_accepted': return { icon:'✅', text:`${who} accepted your trade offer.` };
    case 'trade_declined': return { icon:'✖️', text:`${who} declined your trade offer.` };
    case 'duel_challenge': return { icon:'⚔️', text:`${who} challenged you to a duel — ${escapeHtml(d.set_name||'a pack')}.` };
    case 'duel_accepted': return { icon:'⚔️', text:`${who} accepted your duel challenge — open your pack!` };
    case 'duel_declined': return { icon:'⚔️', text:`${who} declined your duel challenge.` };
    case 'duel_result': return { icon: d.tie ? '🤝' : (d.won ? '🏆' : '💔'), text: d.tie ? `Your duel with ${who} over ${escapeHtml(d.set_name||'a pack')} was a tie.` : (d.won ? `You won your duel with ${who}!` : `You lost your duel with ${who}.`) };
    case 'achievement_earned': return { icon:'🏆', text:`New badge earned: ${escapeHtml(ACHIEVEMENT_META[d.achievement_key]?.label || d.achievement_key)}.` };
    case 'rank_up': return { icon:'⭐', text:`You ranked up to ${escapeHtml(TIER_LADDER.find(t=>t.key===d.tier)?.label || d.tier)}!` };
    case 'referral_joined': return { icon:'🎉', text:`${who} joined using your referral code.` };
...
async function renderSetDetail(setMeta){
  ...
  wrap.innerHTML = `
    <div class="pack-hero">
      ...
      ${!isAdminUser() && myPackTickets[`${ACTIVE_GAME}:${setMeta.id}`] > 0 ? `<div class="pack-ticket-banner">🎟️ You have ${myPackTickets[`${ACTIVE_GAME}:${setMeta.id}`]} free pack ticket${myPackTickets[`${ACTIVE_GAME}:${setMeta.id}`] > 1 ? 's' : ''} for this set — your next open${myPackTickets[`${ACTIVE_GAME}:${setMeta.id}`] > 1 ? 's use them' : ' uses one'} automatically, no charge</div>` : ''}
...
function showSummary(setMeta, pack, openingId){
  ...
  sheet.innerHTML = `
    ...
    ${canShare ? `<button class="btn btn-secondary" style="width:auto; padding:6px 16px; font-size:12.5px; margin:12px auto 0; display:block; opacity:0.8;" id="sum-share-flex">${isHit ? '📸' : '📤'} Share this pull</button>` : ''}

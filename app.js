...
async function getCardsForSet(setId){
  const key = 'cache_cards_v5_' + setId; // v3 -> v4: JP cards were caching with no rarity; v4 -> v5: JP cards with no TCGdex image on either locale now get a pokemontcg.io fallback (see below) — old cached blanks must not be served
  const cached = store.get(key);
  if(cached && Date.now() - cached.t < 1000*60*60*24*7) return cached.data;
  if (setId.startsWith('jp-')) {
    const realId = setId.slice(3);
    try{
      const res = await fetch(`https://api.tcgdex.net/v2/ja/sets/${realId}`);
      ...
      if (jaCards.some(c => !c.image)) {
        try {
          const enRes = await fetch(`https://api.tcgdex.net/v2/en/sets/${realId}`);
          if (enRes.ok) {
            const enData = await enRes.json();
            for (const ec of (enData.cards || [])) {
              if (ec.localId && ec.image) enByLocalId[ec.localId] = ec.image;
            }
          }
      ...
      async function rarityWorker() {
        while (cursor < jaCards.length) {
          ...
          try {
            const cRes = await fetch(`https://api.tcgdex.net/v2/ja/cards/${c.id}`);
            if (cRes.ok) {
      ...
      if (missingRarity.length) {
        ...
        try {
          const enSetRes = await fetch(`https://api.tcgdex.net/v2/en/sets/${realId}`);
          if (enSetRes.ok) {
            ...
            async function enRarityWorker() {
              while (ecursor < enBriefs.length) {
                ...
                try {
                  const ecRes = await fetch(`https://api.tcgdex.net/v2/en/cards/${ec.id}`);
                  if (ecRes.ok) {
      ...
      // for cards TCGdex is still missing. There's no shared card ID
      // between the two APIs for JP cards, so this matches by exact card
      // name (best effort) — good enough since we only need *an* image
      // for the same Pokémon/card, not a byte-identical scan.
      const stillMissingImg = jaCards.filter(c => !c.image && !enByLocalId[c.localId] && c.name);
      let pokeImgByName = {};
      if (stillMissingImg.length) {
        const uniqueNames = [...new Set(stillMissingImg.map(c => c.name))];
        let pcursor = 0;
      ...
      const data = jaCards.map(c => {
        ...
        const pokeFallback = !img ? pokeImgByName[c.name] : null;
        return {
          id: 'jp-' + c.id,
          name: c.name,
...
async function renderSetDetail(setMeta){
  ...
  try{
    ...
    try {
      for(const url of rawUrls) {
        try {
          ...
          if(resolved) validUrls.push(resolved);
        } catch(err) { continue; }
      }
    } finally {
      hideLoader();
    ...
    status.textContent = `Couldn't load pack art (network issue) — retry by reopening this set.`;

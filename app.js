...
function resolveAndAppendSetCard(s, myGen, gridEl) {
  return (async () => {
    ...
    // Prefer release year over the raw API "series" field here: pokemontcg.io
    // groups Base Set, Jungle, Fossil, Team Rocket, Base Set 2, and Wizards
    // Black Star Promos all under the single era name "Base", which reads as
    // if every one of those sets IS Base Set. Year is unambiguous. Sets with
    // no releaseDate (JP tab, One Piece tab, MTG placeholders) fall back to
    // series, same as before this change, so those tabs are unaffected.
    const metaLabel = s.releaseDate ? s.releaseDate.split(/[-/]/)[0] : (s.series || '');

    card.innerHTML = `<img src="" onerror="this.src='data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIxIiBoZWlnaHQ9IjEiPjwvc3ZnPg==';this.classList.add('loaded')" onload="this.classList.add('loaded')" alt=""/><div class="meta">${metaLabel}</div><div class="name-row"><div class="name">${s.name}</div><div class="cost-badge">${costDisplay} cr</div></div>`;
...
async function renderSetDetail(setMeta){
  ...
  try{
    ...
    function appendArtCard(url) {
      if (shownUrls.has(url)) return; // de-dupe, same as the old Set(validUrls)
      shownUrls.add(url);
      if (!firstShown) {
    ...
    await Promise.all(rawUrls.map(async (url) => {
      try {
        ...
        if (resolved) appendArtCard(resolved);
      } catch (e) { /* this candidate failed — others still render on their own */ }
    }));

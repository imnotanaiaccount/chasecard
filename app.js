...
async function ensureSetWarmed(setId, game, cards){
  ...
  const endpoint = game === 'onepiece' ? 'warm-set-cache-op'
    ...
    : isJp ? 'warm-set-cache-jp' : 'warm-set-cache';
...
async function getSetsPokemon(){
  ...
  const data = (enResult.status === 'fulfilled' ? enResult.value : [])
  ...
  return new Promise((resolve) => {
    ...
    overlay.innerHTML = `
      <div style="display:flex; flex-direction:column; align-items:center; gap:14px; animation: slideup 0.3s ease; text-align:center;">
        ...
        ${reward ? `<div style="font-weight:700; font-size:15px; color:var(--gold);">+${reward} Credits</div>` : ''}
...
async function claimLoginStreakNow(){
...
function showStreakCelebration(data, onClose){
  ...
  ov.innerHTML = `
    ...
    <div class="sfx-bottom">
      ...
      ${rankHtml}
      <button type="button" class="btn btn-primary sfx-continue">Continue</button>
    </div>`;
...
async function renderHome(){
  ...
  if(!store.get('seen_welcome')){
    ...
    welcome.innerHTML = `
      ...
      <div style="color:var(--dim); font-size:13.5px; line-height:1.5;">Open virtual Pokémon, Yu-Gi-Oh!, and One Piece booster packs using free credits — no purchase needed. Pick a set below, tear the pack, and see what you pull. Credits are virtual and never cost real money.</div>
    `;

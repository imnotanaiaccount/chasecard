...
const Prewarm = {
  ...
  async _resolvePackArtUrlsUncached(setMeta) {
    ...
    // v8->v9: resolution logic changed (own art now suppresses the redundant
    // ptcg-assets packshots fallback below) — bump so anyone with an
    // existing cached URL list for this set doesn't keep serving the old
    // (duplicated) candidate list for the rest of its TTL.
    const cacheKey = 'packart_urls_v9_' + setMeta.id; 
    const cached = store.get(cacheKey);
...
// sending one RLS will reject.
const PACK_ART_CACHE_VERSION = 'v9'; // matches the packart_urls_v9_ local cache key below — bump both together if the resolution pipeline changes

...
const CONFIG = {
  ...
  ECONOMY: {
    ...
    // Kept fully separate from `is_premium` (account-area/referral
    // copy below), the one genuinely admin-granted, non-earned status.
  },
};
...
const ACHIEVEMENT_ICON_PATHS = {
  ...
  supporter:     '<polygon points="12,2 14.5,9.5 22,12 14.5,14.5 12,22 9.5,14.5 2,12 9.5,9.5" fill="currentColor" stroke="none"/>'
};
...
const ACHIEVEMENT_COLORS = {
  ...
  popular: '#f472b6', supporter: '#f472b6'
};
...
const ACHIEVEMENT_META = {
  ...
  supporter:     { label: 'Supporter',     desc: 'Gifted by the team for supporting the app (e.g. a tip)' },
};
let myAchievements = [];
...
function renderTopbar(){
  ...
  // real account) and admins (already shown their own distinct
  // "System Admin" status on Profile — a rank chip here would just be
  // a second, redundant status readout for them specifically).
  let rankChip = '';
...
function renderAllBadgesPanel(userProfile){
  ...
  Object.keys(ACHIEVEMENT_META).forEach(key => {
    if (key === 'supporter') return; // manually admin-granted, not something to "work toward" — listing it here would be misleading
    const meta = ACHIEVEMENT_META[key];
  ...
  const accountHtml = `
    <div class="account-card">
      ...
      <div class="account-details">
        ...
        ${userProfile?.is_premium ? `<p><strong>Account type:</strong> Supporter (gifted by the team)</p>` : ''}
...
async function renderProfile() {
  ...
  wrap.innerHTML = `
      ...
      <div id="admin-panel" style="display:none; margin-top:30px; padding:18px; background:var(--panel-2); border:1px solid var(--vip-gold-dim); border-radius:14px; box-shadow: 0 4px 15px rgba(255, 233, 184, 0.1);">
         ...
         <div style="margin-top:18px; padding-top:16px; border-top:1px solid var(--edge);">
             <p class="hint" style="margin-bottom:4px;">Manually grant the Supporter badge to the selected user above (e.g. for a tip) — purely cosmetic, grants no credits or tier.</p>
             <p class="hint" id="admin-supporter-target-label" style="margin-bottom:10px; font-weight:bold; color:var(--text);">Target: —</p>
             <button class="btn btn-secondary" id="admin-grant-supporter-btn" style="width:100%; display:flex; align-items:center; justify-content:center; gap:8px;"><span style="display:inline-flex; color:${ACHIEVEMENT_COLORS.supporter};">${badgeIconSVG(ACHIEVEMENT_ICON_PATHS.supporter, 18)}</span>Grant Supporter Badge</button>
  ...
  setTimeout(async () => {
    ...
    if (session && profile?.is_admin) {
        ...
        const supporterTargetLabel = $('#admin-supporter-target-label', wrap);
        ...
        function updateSelectedUserPanel(identifier) {
            ...
            if (!u) {
                ...
                if (supporterTargetLabel) supporterTargetLabel.textContent = 'Target: —';
                return;
            }
            ...
            if (supporterTargetLabel) supporterTargetLabel.textContent = `Target: ${u.username || u.email || u.id}`;
        ...
        if (grantSupporterBtn) {
            grantSupporterBtn.addEventListener('click', async () => {
                ...
                const targetLabel = adminUsersById[targetIdentifier]?.username || adminUsersById[targetIdentifier]?.email || targetIdentifier;
                try {
                    ...
                    achievementMsgBox.textContent = `Supporter badge granted to ${targetLabel}!`;
                } catch(e) {
                    achievementMsgBox.style.color = 'var(--danger)';
                    achievementMsgBox.textContent = e.message;
                }
                grantSupporterBtn.disabled = false; grantSupporterBtn.innerHTML = `<span style="display:inline-flex; color:${ACHIEVEMENT_COLORS.supporter};">${badgeIconSVG(ACHIEVEMENT_ICON_PATHS.supporter, 18)}</span>Grant Supporter Badge`;
            });
        }
    } else {
...
function openGetCreditsModal(lowBalance=false){
  ...
  sheet.innerHTML = `
    ...
    <div class="hint" style="margin-bottom:18px;">You both get +${refBonus.toLocaleString()} credits when they sign up.${profile?.is_premium ? ' (2× Supporter bonus applied)' : ''}</div>
    ${shareSectionHTML()}
    ${profile?.is_premium ? `
    <div class="bundle" style="flex-direction:column;align-items:stretch;gap:8px; margin-top:12px;">
      <div>
        <div class="amt" style="display:flex; align-items:center; gap:6px; color:var(--gold);">Supporter status</div>
        <p class="sub">Gifted by the site admin — separate from your earned Rank, and doubles your referral bonus.</p>
      </div>
    </div>` : ''}
  `;
  overlay.appendChild(sheet); document.body.appendChild(overlay);

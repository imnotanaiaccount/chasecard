...
/* ============================================================
   ...
   that tier (matches CONFIG.ECONOMY.PREMIUM_TIERS labels above).
   ============================================================ */
const TIER_LADDER = [
  { key:'free',    label:'Free',    icon:'🌱', packs:0,    referrals:0,  dailyBonus:0,     color:'#94a3b8' },
  { key:'starter', label:'Starter', icon:'🥉', packs:10,   referrals:1,  dailyBonus:500,   color:'#cd7f32' },
  { key:'plus',    label:'Plus',    icon:'🥇', packs:50,   referrals:3,  dailyBonus:1000,  color:'#eab308' },
  { key:'pro',     label:'Pro',     icon:'🔥', packs:150,  referrals:7,  dailyBonus:2000,  color:'#3b82f6' },
  { key:'elite',   label:'Elite',   icon:'💎', packs:400,  referrals:15, dailyBonus:4000,  color:'#8b5cf6' },
  { key:'vip',     label:'VIP',     icon:'👑', packs:1000, referrals:30, dailyBonus:10000, color:'#f59e0b' },
];
...
/* ============================================================
   ...
   - TCGplayer: runs its affiliate program through Impact
     (impact.com) rather than a simple query param. After you're
     approved, Impact gives you a tracking-link template that
     wraps any destination URL. Paste that template into
     TCGPLAYER_WRAPPER_TEMPLATE with {URL} where the destination
     goes (e.g. 'https://tcgplayer.pxf.io/c/XXXXX/YYYY/ZZZZ?u={URL}').
     Until you do, TCGplayer links are skipped (not shown broken).
   - GameStop: also wrapper-style (currently via Rakuten Advertising),
     same pattern as TCGplayer — apply, get your tracking template,
     paste it into GAMESTOP_WRAPPER_TEMPLATE with {URL} in it.
     GameStop's bar is lower than Amazon's — no traffic/sales minimum
     stated, mainly a content-relevance check — worth trying directly.
...
const AFFILIATE = {
  ...
  TCGPLAYER_WRAPPER_TEMPLATE: '', // Impact tracking template with {URL} placeholder
  GAMESTOP_WRAPPER_TEMPLATE: '',  // Rakuten (or current network) tracking template with {URL} placeholder
  ...
  tcgplayerUrl(query){
    if(!this.TCGPLAYER_WRAPPER_TEMPLATE) return null;
    const dest = `https://www.tcgplayer.com/search/all/product?q=${encodeURIComponent(query)}`;
    return this.TCGPLAYER_WRAPPER_TEMPLATE.replace('{URL}', encodeURIComponent(dest));
  },
  gamestopUrl(query){
  ...
  // callers render these, they don't build URLs themselves.
  linksFor(query){
    const out = [];
    const tcg = this.tcgplayerUrl(query);
    const ebay = this.ebayUrl(query);
    const amazon = this.amazonUrl(query);
    const gamestop = this.gamestopUrl(query);
    if(tcg) out.push({ label: 'TCGplayer', url: tcg });
    if(ebay) out.push({ label: 'eBay', url: ebay });
    if(amazon) out.push({ label: 'Amazon', url: amazon });
    if(gamestop) out.push({ label: 'GameStop', url: gamestop });
    return out;
  },
};
...
function renderAffiliateButtons(query){
  const links = AFFILIATE.linksFor(query);
...
.tier-ladder-row .icon { font-size: 15px; width: 20px; text-align: center; flex-shrink: 0; }
  ...
  overlay.innerHTML = `
      ...
      <div style="position:relative; width:90%; max-width:400px; perspective:1200px; display:flex; flex-direction:column; align-items:center;">
          ...
          ${cardObj ? `
            ...
            ${renderAffiliateButtons(cardObj.name)}
          ` : ''}
      </div>
  `;
...
const ACHIEVEMENT_META = {
  first_pull:    { icon: '🎴', label: 'First Pull',    desc: 'Opened your first pack' },
  pack_rat:      { icon: '📦', label: 'Pack Rat',      desc: 'Opened a bunch of packs' },
  century_club:  { icon: '💯', label: 'Century Club',  desc: 'Opened 100 packs' },
  first_trade:   { icon: '🤝', label: 'First Trade',   desc: 'Completed your first trade' },
  dealmaker:     { icon: '🔁', label: 'Dealmaker',     desc: 'Completed several trades' },
  duelist:       { icon: '⚔️', label: 'Duelist',       desc: 'Completed your first duel' },
  win_streak_5:  { icon: '🔥', label: 'Win Streak',    desc: 'Won 5 duels in a row' },
  dealer:        { icon: '💰', label: 'Dealer',        desc: 'Sold cards on the market' },
  high_roller:   { icon: '📈', label: 'High Roller',   desc: 'Sold cards for a lot of credits' },
  set_complete:  { icon: '✅', label: 'Set Complete',  desc: 'Completed a full set' },
  collector:     { icon: '🗂️', label: 'Collector',     desc: 'Completed 5 full sets' },
  whale_watcher: { icon: '💎', label: 'Whale Watcher', desc: 'Pulled a top-tier rare card' },
  popular:       { icon: '👥', label: 'Popular',       desc: 'Referred several friends' },
  supporter:     { icon: '🌟', label: 'Supporter',     desc: 'Gifted by the team for supporting the app (e.g. a tip)' },
};
let myAchievements = [];
...
function renderTierProgressBanner(userProfile) {
  ...
  const subLine = p.maxed
    ...
    : `${Math.round(p.percent)}% of the way to ${next.icon} ${next.label}`;
  ...
  return `
    <div class="tier-progress-banner" style="--tier-color:${current.color};">
      <div class="tier-progress-top">
        <span class="tier-progress-icon">${current.icon}</span>
...
function renderBadgesSection(userProfile) {
  ...
  if (userProfile?.founding_member) {
    items.push({
      icon: '🏆',
      label: `Founding Member #${userProfile.founding_member_number}`,
      desc: '2× earned credits, forever',
      gold: true
    });
  }
  myAchievements.forEach(key => {
    const meta = ACHIEVEMENT_META[key];
    if (meta) items.push({ icon: meta.icon, label: meta.label, desc: meta.desc, gold: false });
  });
  ...
  return `
    <div style="margin-top:12px;">
      ...
      <div style="display:flex; flex-wrap:wrap; gap:6px; margin-top:6px;">
        ${items.map(b => `<span class="account-badge${b.gold ? ' badge-founding' : ' badge-default'}" title="${escapeHtml(b.desc || b.label)}">${b.icon} ${escapeHtml(b.label)}</span>`).join('')}
      </div>
    </div>`;
}
...
function renderAccountArea(user, userProfile) {
  ...
  const accountHtml = `
    <div class="account-card">
      ${!user ? '' : isAdmin
        ? `<div class="admin-status-line"><span>🛠️</span><span>System Admin — unlimited credits &amp; full access</span></div>`
        : renderTierProgressBanner(userProfile)}
      ...
      <div class="account-details">
        ...
        ${userProfile?.is_premium ? `<p><strong>Account type:</strong> Premium (Admin-Granted)</p>` : ''}
...
async function renderProfile() {
  ...
  wrap.innerHTML = `
      ...
      <div id="admin-panel" style="display:none; margin-top:30px; padding:18px; background:var(--panel-2); border:1px solid var(--vip-gold-dim); border-radius:14px; box-shadow: 0 4px 15px rgba(255, 233, 184, 0.1);">
         ...
         <div style="margin-top:18px; padding-top:16px; border-top:1px solid var(--edge);">
             <p class="hint" style="margin-bottom:10px;">Manually grant the 🌟 Supporter badge to the selected user above (e.g. for a tip) — purely cosmetic, grants no credits or tier.</p>
             <button class="btn btn-secondary" id="admin-grant-supporter-btn" style="width:100%;">🌟 Grant Supporter Badge</button>
             <div id="admin-achievement-msg" class="hint" style="margin-top:10px; text-align:center; font-weight:bold; min-height:16px;"></div>
         </div>
         ...
         <div style="margin-top:18px; padding-top:16px; border-top:1px solid var(--edge);">
             ...
             <button class="btn btn-secondary" id="admin-export-subs-btn" style="width:100%;">📧 Export Subscriber List</button>
         ...
         <div style="margin-top:18px; padding-top:16px; border-top:1px solid var(--edge);">
             ...
             <button class="btn btn-secondary" id="admin-load-analytics-btn" style="width:100%;">📊 Load Analytics</button>
             <div id="admin-analytics-output" style="margin-top:10px;"></div>
             <button class="btn btn-secondary" id="admin-load-funnel-btn" style="width:100%; margin-top:10px;">🎯 Founding Banner → Signup Funnel</button>
  ...
  if (AFFILIATE.linksFor('x').length || AFFILIATE.SOVRN_API_KEY) {
    ...
    disclosure.innerHTML = `<div class="hint" style="font-size:10px;">Some card/booster links in this app are affiliate links — we may earn a commission on qualifying purchases at no extra cost to you.</div>`;
  ...
  setTimeout(async () => {
    ...
    if (session && profile?.is_admin) {
        ...
        function updateSelectedUserPanel(identifier) {
            ...
            if (!u) { selectedUserBox.style.display = 'none'; return; }
        ...
        if (exportSubsBtn) {
            exportSubsBtn.addEventListener('click', async () => {
                ...
                exportSubsBtn.disabled = false; exportSubsBtn.textContent = '📧 Export Subscriber List';
            });
        }
        ...
        if (loadAnalyticsBtn) {
            loadAnalyticsBtn.addEventListener('click', async () => {
                ...
                loadAnalyticsBtn.disabled = false; loadAnalyticsBtn.textContent = '📊 Load Analytics';
            });
        }
        ...
        if (loadFunnelBtn) {
            loadFunnelBtn.addEventListener('click', async () => {
                ...
                loadFunnelBtn.disabled = false; loadFunnelBtn.textContent = '🎯 Founding Banner → Signup Funnel';
            });
        }

        const grantSupporterBtn = $('#admin-grant-supporter-btn', wrap);
        const achievementMsgBox = $('#admin-achievement-msg', wrap);
        if (grantSupporterBtn) {
            grantSupporterBtn.addEventListener('click', async () => {
                const targetIdentifier = userSelect.value;
                if(!targetIdentifier) {
                    achievementMsgBox.style.color = 'var(--danger)';
                    achievementMsgBox.textContent = 'Please select a valid user.';
                    return;
                }
                grantSupporterBtn.disabled = true; grantSupporterBtn.textContent = 'Granting...';
                try {
                    const { error } = await sb.rpc('admin_grant_achievement', { target_identifier: targetIdentifier, p_key: 'supporter' });
                    if(error) throw error;
                    achievementMsgBox.style.color = 'var(--cyan)';
                    achievementMsgBox.textContent = 'Supporter badge granted!';
                } catch(e) {
                    achievementMsgBox.style.color = 'var(--danger)';
                    achievementMsgBox.textContent = e.message;
                }
                grantSupporterBtn.disabled = false; grantSupporterBtn.textContent = '🌟 Grant Supporter Badge';
            });
        }
    } else {
...
function badgePillHTML(icon, label, desc, extraClass, extraStyle) {
  return `<span class="account-badge${extraClass ? ' ' + extraClass : ''}"${extraStyle ? ` style="${extraStyle}"` : ''} title="${escapeHtml(desc || label)}">${icon} ${escapeHtml(label)}</span>`;
}
...
function renderPublicBadgesRow(tierRow, achievementKeys) {
  ...
  if (tierRow) {
    ...
    items.push(badgePillHTML(t.icon, `${t.label} Tier`, `${t.label} tier`, null, `background:${t.color}26; border:1px solid ${t.color}; color:${t.color};`));
    if (tierRow.founding_member) {
      items.push(badgePillHTML('🏆', `Founding Member #${tierRow.founding_member_number}`, '2× earned credits, forever', 'badge-founding'));
    }
  }
  (achievementKeys || []).forEach(key => {
    const meta = ACHIEVEMENT_META[key];
    if (meta) items.push(badgePillHTML(meta.icon, meta.label, meta.desc, 'badge-default'));
  });
  if (!items.length) return '';
  return `<div id="public-badges-row" style="display:flex; flex-wrap:wrap; gap:6px; margin:4px 0 14px;">${items.join('')}</div>`;
}
...
async function renderSetDetail(setMeta){
  ...
  wrap.innerHTML = `
    <div class="pack-hero">
      ...
      ${renderAffiliateButtons(`${setMeta.name} booster box`)}
    </div>
  `;
  app.appendChild(wrap);

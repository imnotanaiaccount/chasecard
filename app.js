...
async function renderSetDetail(setMeta){
  ...
  try{
    ...
    // Sequential, not Promise.all: appendArtCard's hash check has to see the
    // PRIOR card's hash before deciding on the next one, or two duplicate
    // images resolving at nearly the same instant would both sail past the
    // shownHashes check before either had been added. Candidates still render
    // progressively (each appears as soon as it's ready) — this only
    // serializes the (fast, local) hashing step, not the network fetches,
    // since ImgCache.get() below still runs those independently up front.
    if (packArtDebugOn) packArtDebugLog(`${rawUrls.length} raw candidate URLs for ${setMeta.id}: ${JSON.stringify(rawUrls)}`);
    const resolvedInOrder = await Promise.all(rawUrls.map(async (url) => {
      ...
      shownUrls.add(url);
      try {
        const r = await ImgCache.get(url, true);
    ...
    for (const resolved of resolvedInOrder) {

...
/* ============================================================
   ...
   ============================================================ */
const TIER_LADDER = [
  { key:'free',    label:'Rookie',  packs:0,    referrals:0,  dailyBonus:0,     color:'#94a3b8' },
  { key:'starter', label:'Starter', packs:10,   referrals:1,  dailyBonus:500,   color:'#cd7f32' },
  { key:'plus',    label:'Plus',    packs:50,   referrals:3,  dailyBonus:1000,  color:'#eab308' },
  { key:'pro',     label:'Pro',     packs:150,  referrals:7,  dailyBonus:2000,  color:'#3b82f6' },
  { key:'elite',   label:'Elite',   packs:400,  referrals:15, dailyBonus:4000,  color:'#8b5cf6' },
  { key:'vip',     label:'VIP',     packs:1000, referrals:30, dailyBonus:10000, color:'#f59e0b' },
];
...
const FOUNDING_ICON_PATH = '<polygon points="6,2 18,2 18,16 12,12 6,16"/>';
const FOUNDING_COLOR = '#ffd700';
...
const FUNNEL_ICON_PATH = '<polygon points="3,4 21,4 14,13 14,20 10,17 10,13"/>';
...
const AFFILIATE = {
  ...
  TCGPLAYER_WRAPPER_TEMPLATE: '', // ONLY needed for a manual Impact tracking-link template. Leave blank if index.html has the Impact Universal Tracking Tag (it does, as of this comment) — that auto-wraps the plain URL below on its own.
...
function putSharedSetCache(setId, cards) {
  ...
  // Non-JP sets: written exclusively through the warm-set-cache Edge
  // Function now (see its own header comment) — it re-resolves the set
  // itself server-side via the service role, rather than trusting
...
function showCardFullscreen(imgSrc, cardObj){
  ...
  overlay.innerHTML = `
      <button id="card-fullscreen-close-btn" aria-label="Close" style="position:fixed; top:16px; right:16px; z-index:301; width:40px; height:40px; border-radius:50%; border:none; background:rgba(0,0,0,0.6); color:#fff; font-size:22px; line-height:1; cursor:pointer; display:flex; align-items:center; justify-content:center;">&times;</button>
...
const ImgCache = {
  ...
  async _fetchRawBlob(url) {
    try {
      ...
      let res = await fetch(mirrorUrl, { mode: 'cors', credentials: 'omit' }).catch(() => null);
      if (!res || !res.ok) {
        if (url.startsWith('pokewallet://images/')) {
          ...
          res = await fetch(url, { mode: 'cors', credentials: 'omit' }).catch(() => null);
        }
      }
...
function putSharedSetCacheOnePiece(setId) {
  // Unlike Pokémon's putSharedSetCache, this doesn't send the client's
  // own resolved cards — warm-set-cache-op does its own resolution
  // server-side (see that function's header comment: simple enough,
  // unlike the JP pipeline, to safely reimplement directly rather than
  // needing a verification gateway). This just tells it which set_id to
  // warm. Guests can't usefully call it either way (no session), and a
  // slow/failed warm shouldn't hold up this user, who already has their
  // own locally-resolved copy regardless.
  if (!session) return;
  ...
  try{
    ...
    if(res.ok){
      ...
      if(arr && arr.length && arr[0]?.card_set_id){
        ...
        putSharedSetCacheOnePiece(setId);
        return data;
      }
    }
...
function openAuthModal(resumeSetMeta = null){
  ...
  function renderMainView() {
    sheet.innerHTML = `
      ...
      <div id="founding-member-banner" style="display:none; margin:10px 0 4px; padding:9px 12px; background:linear-gradient(135deg, rgba(255,215,0,0.18), rgba(255,215,0,0.05)); border:1px solid var(--vip-gold-dim); border-radius:10px; text-align:center; font-size:12.5px; color:var(--vip-gold); font-weight:700;"></div>
    ...
    sb.rpc('get_founding_member_stats').then(({ data, error }) => {
      if (error || !data || !data.length) return;
      const remaining = data[0].remaining;
      const banner = $('#founding-member-banner', sheet);
      if (!banner || remaining <= 0) return;
      banner.style.display = 'block';
      banner.textContent = `Only ${remaining.toLocaleString()} Founding Member spots left — 2x earned credits, forever`;
      track('founding_banner_shown', { remaining });
    }).catch(() => {});
...
function renderBadgesSection(userProfile) {
  ...
  // Admin accounts get the Founding badge shown too, even when
  // founding_member isn't actually set on the row — same reasoning as
  // "all badges" below: an admin account should be able to see/display
  // every badge that exists, cosmetically, without needing the real
  // gameplay/DB conditions that gate it for normal accounts.
  if (userProfile?.founding_member || admin) {
    tiles.push(badgeTileHTML(
      badgeIconSVG(FOUNDING_ICON_PATH, 32),
      userProfile?.founding_member_number ? `Founding #${userProfile.founding_member_number}` : 'Founding Member',
      '2× earned credits, forever',
      FOUNDING_COLOR,
      true
    ));
  }
  // Admin sees every achievement badge as earned (each keeping its own
  // distinct ACHIEVEMENT_COLORS color) instead of only whatever this
  // account's real `achievements` rows happen to contain — which, for
  // an admin account that doesn't play through normal pack/trade/duel
  // flows, was often nothing, or just the single blue System Admin
  // icon shown elsewhere on the page (easy to mistake for "badges are
  // all blue").
  const earnedKeys = admin ? Object.keys(ACHIEVEMENT_META) : myAchievements;
...
function renderBattleDuel(deck){
  ...
  wrap.innerHTML = `
    ...
    <div class="battle-arena">
      <div class="battle-side">
        <div class="battle-side-label">Opponent · ${state.ai.ko}/${BATTLE_KO_TARGET} KOs</div>
        <div id="ai-active-slot" class="battle-active-slot"></div>
      </div>
      <div class="battle-side">
        <div class="battle-side-label">You · ${state.player.ko}/${BATTLE_KO_TARGET} KOs</div>
  ...
  attackBtn.addEventListener('click', () => {
    ...
    if (!state.winner) battleAiTakeTurn(state); // AI attacks back same beat — keeps a duel short and readable
    redraw();
  });
...
function renderAllBadgesPanel(userProfile){
  ...
  if (userProfile) {
    // Show the account's REAL founding_member flag first — always. An
    // admin account can genuinely be a founding member (e.g. the
    // developer's own account, created among the first 1,000 rows in
    // `profiles`), and hardcoding "not applicable" here used to hide
    // that regardless of what the DB actually said, which read as a
    // bug ("I should have this badge") rather than the intended
    // messaging. Only once we know it's NOT earned do we show the
    // admin-specific "wouldn't do anything for you anyway" caveat,
    // since the 2x-earned-credits payoff genuinely is moot for an
    // unlimited-credits account that never earned this badge.
    const hasFounding = !!userProfile.founding_member;
    if (hasFounding || isAdmin) {
      // Admin accounts show Founding Member as earned too (see
      // renderBadgesSection) — the 2x-credits payoff is moot for an
      // unlimited-credits account, but the badge itself still displays.
      rows.push(`
        <div class="tier-ladder-row current-tier-row" style="--tier-color:${FOUNDING_COLOR};">
          <span class="icon" style="color:${FOUNDING_COLOR};">${badgeIconSVG(FOUNDING_ICON_PATH, 18)}</span>
          <span class="name">Founding Member</span>
          <span class="req">${userProfile.founding_member_number ? `Founder #${userProfile.founding_member_number} — ` : ''}${isAdmin && !hasFounding ? '2× earned credits (no effect — unlimited credits already)' : '2× earned credits, forever'}</span>
          <span style="font-size:10px; font-weight:800; color:${FOUNDING_COLOR}; flex-shrink:0;">EARNED</span>
        </div>`);
    } else {
      rows.push(`
        <div class="tier-ladder-row" style="opacity:0.65; --tier-color:${FOUNDING_COLOR};">
          <span class="icon" style="color:${FOUNDING_COLOR};">${badgeIconSVG(FOUNDING_ICON_PATH, 18)}</span>
          <span class="name">Founding Member</span>
          <span class="req">Among the first 1,000 accounts — 2× earned credits, forever</span>
        </div>`);
    }
  }
  ...
  Object.keys(ACHIEVEMENT_META).forEach(key => {
    const meta = ACHIEVEMENT_META[key];
    // Admin sees every achievement as earned here too, matching the
    // badges grid above (renderBadgesSection).
    const earned = earnedSet.has(key) || isAdmin;
  ...
  return `
    <div style="margin-top:14px;">
      ...
      <div id="all-badges-panel" class="tier-details-panel" style="display:none;">
        ...
        <p style="margin:6px 0 10px;">Dimmed ones aren't earned yet. Some come from gameplay milestones, Founding Member is limited to the first 1,000 accounts.</p>
        ${rows.join('')}
      </div>
    </div>`;
}
      ...
      <div class="account-details">
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; margin-top:14px; padding-top:12px; border-top:1px solid var(--edge);">
          <div style="min-width:0;">
            ...
            <div style="font-size:11.5px; color:var(--dim); margin-top:2px;">When on, other users can see your tier, founding-member status, achievements, and collection. Your email and credits are never shown.</div>
          </div>
...
async function renderProfile() {
      ...
      <div id="admin-panel" style="display:none; margin-top:30px; padding:18px; background:var(--panel-2); border:1px solid var(--vip-gold-dim); border-radius:14px; box-shadow: 0 4px 15px rgba(255, 233, 184, 0.1);">
         ...
         <div style="margin-bottom:12px;">
             ...
             <select id="admin-target-tier-select" style="width:100%; padding:12px 14px; border-radius:12px; border:1px solid var(--edge); background:var(--panel); color:var(--text); font-family:var(--font-body);">
                 <option value="free">Rookie (base rank, 0 daily credits)</option>
                 <option value="starter">Starter (500 daily credits)</option>
                 <option value="plus">Plus (1,000 daily credits)</option>
                 <option value="pro">Pro (2,000 daily credits)</option>
                 <option value="elite">Elite (4,000 daily credits)</option>
                 <option value="vip">VIP Member (10,000 daily credits)</option>
         ...
         <div style="margin-top:18px; padding-top:16px; border-top:1px solid var(--edge);">
             <p class="hint" style="margin-bottom:10px;">Last 7 days — signups, pack opens, affiliate clicks, referrals, and how many people saw the Founding Member banner.</p>
             ...
             <button class="btn btn-secondary" id="admin-load-funnel-btn" style="width:100%; margin-top:10px; display:flex; align-items:center; justify-content:center; gap:8px;"><span style="display:inline-flex;">${badgeIconSVG(FUNNEL_ICON_PATH, 18)}</span>Founding Banner → Signup Funnel</button>
             <div id="admin-funnel-output" style="margin-top:10px;"></div>
         </div>
      </div>
  ...
  setTimeout(async () => {
    ...
    if (session && profile?.is_admin) {
        ...
        const loadFunnelBtn = $('#admin-load-funnel-btn', wrap);
        const funnelOutput = $('#admin-funnel-output', wrap);
        if (loadFunnelBtn) {
            loadFunnelBtn.addEventListener('click', async () => {
                loadFunnelBtn.disabled = true; loadFunnelBtn.textContent = 'Loading...';
                try {
                    const { data, error } = await sb.rpc('admin_get_funnel', { p_days: 30 });
                    if (error) throw error;
                    const row = data?.[0];
                    if (!row || row.banner_shown === 0) {
                        funnelOutput.innerHTML = `<div class="hint" style="text-align:center;">No banner impressions in the last 30 days yet.</div>`;
                    } else {
                        funnelOutput.innerHTML = `
                          <div style="border:1px solid var(--edge); border-radius:10px; padding:12px; font-size:13px;">
                            <div style="display:flex; justify-content:space-between; padding:4px 0;"><span>Saw the banner</span><span style="font-weight:800;">${Number(row.banner_shown).toLocaleString()}</span></div>
                            <div style="display:flex; justify-content:space-between; padding:4px 0;"><span>Signed up after seeing it</span><span style="font-weight:800; color:var(--vip-gold);">${Number(row.signups_after_banner).toLocaleString()}</span></div>
                            <div style="display:flex; justify-content:space-between; padding:4px 0;"><span>Signed up without seeing it</span><span style="font-weight:800;">${Number(row.signups_no_banner).toLocaleString()}</span></div>
                            <div style="display:flex; justify-content:space-between; padding:8px 0 0; margin-top:6px; border-top:1px solid var(--edge);"><span>Conversion rate</span><span style="font-weight:800; color:var(--cyan);">${row.conversion_rate != null ? row.conversion_rate + '%' : '—'}</span></div>
                          </div>
                          <div class="hint" style="font-size:10.5px; margin-top:6px;">Matches visitors by an anonymous device ID set in the browser — won't catch someone who saw the banner on one device and signed up on another.</div>
                        `;
                    }
                } catch (e) {
                    funnelOutput.innerHTML = `<div class="hint" style="color:var(--danger); text-align:center;">${escapeHtml(e.message)}</div>`;
                }
                loadFunnelBtn.disabled = false; loadFunnelBtn.innerHTML = `<span style="display:inline-flex;">${badgeIconSVG(FUNNEL_ICON_PATH, 18)}</span>Founding Banner → Signup Funnel`;
            });
        }
...
// tab uses renderBadgesSection instead, which also handles admin/founding
// styling for the owner's view). Relies on the get_public_profile_badges
// RPC since profiles' own RLS only allows reading your own row — this
// RPC deliberately exposes just tier/founding-member fields, gated on
// that profile's is_public flag, nothing else (no email/credits/etc).
function renderPublicBadgesRow(tierRow, achievementKeys) {
  ...
  if (tierRow) {
    ...
    if (tierRow.founding_member) {
      tiles.push(badgeTileHTML(
        badgeIconSVG(FOUNDING_ICON_PATH, 32),
        `Founding #${tierRow.founding_member_number}`,
        '2× earned credits, forever',
        FOUNDING_COLOR,
        true
      ));
...
// Flat per-pack credit value — used ONLY for the guest-mode share bonus
// below now (guests have no server row to grant a real ticket against,
// see the comment in claimShareBonus). For logged-in accounts, the
// welcome/share/referral pack rewards now grant real per-set redemption
// tickets (see the pack_tickets table + open_pack in the SQL) instead of
// a credit-equivalent amount — a ticket can only ever open the specific
// set it names, unlike a flat credit grant.
const PROMO_PACK_VALUE = 180;
const SHARE_PACK_BONUS = PROMO_PACK_VALUE; // guest-mode-only equivalent for 1 Base Set 2 pack
...
async function claimShareBonus(platform, btn){
  ...
  try{
    if(guestMode){
      // Guests have no server row to grant a real redemption ticket
      // against — their whole pack economy is already simulated
      // client-side (see beginOpen), so a credit-equivalent is the
      // closest sensible thing here, same as before.
      const gs = getGuestState();
      gs.credits = (Number(gs.credits) || 0) + SHARE_BONUS + SHARE_PACK_BONUS;
      gs.sharedPlatforms = [...(gs.sharedPlatforms || []), platform];
      setGuestState(gs);
      const creditCountEl = $('#credit-count');
      if(creditCountEl) creditCountEl.textContent = gs.credits;
      SFX.coin();
      toast(`+${(SHARE_BONUS + SHARE_PACK_BONUS).toLocaleString()} credits — thanks for sharing!`);
    } else {
      ...
      // Separate RPC/table for the pack ticket (see claim_share_pack_bonus
      // in the SQL) rather than folding it into claim_share_bonus above —
      // that function already existed before this feature and its exact
      // internals aren't visible here, so adding to it directly risked
      // silently breaking whatever validation it already does. This way
      // the original bonus is untouched and the new one is fully additive.
      // Grants a real Base Set 2 pack ticket now (see open_pack) — not a
      // credit-equivalent — so it can only ever be spent on that set.
      try { await sb.rpc('claim_share_pack_bonus', { p_platform: platform }); await refreshPackTickets(); }
      catch (e2) { /* original share bonus still succeeded either way */ }
      profile.shared_platforms = [...(profile.shared_platforms || []), platform];
      const creditCountEl = $('#credit-count');
      if(creditCountEl) creditCountEl.textContent = isAdminUser() ? '∞' : newBalance;
      SFX.coin();
      toast(`+${SHARE_BONUS.toLocaleString()} credits + 1 free Base Set 2 pack — thanks for sharing!`);
    }
...
function shareSectionHTML(){
  ...
  return `
    ...
    <div class="hint" style="margin-bottom:10px;">Share Chase Cards for +${SHARE_BONUS.toLocaleString()} credits plus a free Base Set 2 pack ticket (once per platform).</div>

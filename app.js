...
/* Facebook-style red notification dot on a bottom-nav tab icon — see
   hasUnclaimedStreak/hasNewFollowers/hasNewAchievementBadge and
   renderTabs(). */
.tab-notif-dot {
  position: absolute;
  top: -2px;
...
let myAchievements = [];
// --- Nav badge state (red notification dots on the Profile/Collections
// tabs) --- myFollowerCount is refreshed fire-and-forget from
// loadProfile(); everything else here just reads already-loaded state
// (profile, myAchievements), so these stay cheap synchronous checks
// safe to call on every renderTabs() pass.
let myFollowerCount = null;
...
function hasNewFollowers(){
  if(myFollowerCount == null) return false;
  return myFollowerCount > (store.get('last_seen_follower_count') || 0);
}
function markFollowersSeen(){
  if(myFollowerCount != null) store.set('last_seen_follower_count', myFollowerCount);
}
async function refreshFollowerCount(){
  if(!session?.user || guestMode){ myFollowerCount = null; return; }
  try{
    const { count } = await sb.from('follows').select('*', { count:'exact', head:true }).eq('following_id', session.user.id);
    myFollowerCount = count || 0;
  }catch(e){ /* leave stale/null — next successful call fixes it */ }
}
function hasNewAchievementBadge(){
  if(!session?.user || guestMode) return false;
  const seen = store.get('seen_achievement_keys') || [];
  return myAchievements.some(k => !seen.includes(k));
}
function markAchievementsSeen(){
  store.set('seen_achievement_keys', myAchievements.slice());
}
...
async function loadProfile(attempt=1){
  ...
  }
  {
    ...
    refreshFollowerCount().then(refreshTabBadges);
...
function render(name, params={}){
  ...
  if(name==='collection'){ renderCollection(); markAchievementsSeen(); }
  if(name==='search') renderSearch();
  if(name==='profile'){ renderProfile(); markFollowersSeen(); }
...
// Swaps just the tab bar in place for a fresh badge-dot check, without
// re-rendering whatever screen is currently showing (a full render()
// call would wipe/rebuild the entire active screen just to update a
// notification dot — fine for navigation, too disruptive to fire
// automatically whenever background data like the follower count
// happens to resolve).
function refreshTabBadges(){
  if(!app) return;
  const old = app.querySelector('.tabs');
  if(old) old.replaceWith(renderTabs());
}
function renderTabs(){
  ...
  items.forEach(it=>{
    ...
    // Facebook-style red dot: Profile lights up for an unclaimed daily
    // streak or a new follower since you last opened Profile;
    // Collections lights up for a newly-earned badge you haven't seen
    // there yet. Both are cheap synchronous reads (see state/helpers
    // near myAchievements above) — nothing async happens on every tab
    // redraw.
    const showBadge = (it.key === 'profile' && (hasUnclaimedStreak() || hasNewFollowers()))
      || (it.key === 'collection' && hasNewAchievementBadge());

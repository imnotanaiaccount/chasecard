...
/* ============================================================
   ...
   pull/collection pipeline every other game uses. NOT yet wired
   into Pack Duels or the Set Checklist picker (openDuelProposer's
   renderStep2 / openSetChecklistPicker's pickerTabs) — both need a
   set's card pool pre-cached server-side in `set_card_cache` via a
   warm-set-cache-* Edge Function (see warm-set-cache-op for the
   template), which doesn't exist for this game yet. Also not in
   getSharedSetCache/putSharedSetCache's cross-user cache for the
   same reason — every browser resolves a set for itself, cached
   only locally (store, below), same tradeoff getSetsOnePiece made
   before warm-set-cache-op existed.

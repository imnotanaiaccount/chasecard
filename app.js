...
// all. This asks once, right after a brand-new Google signup, instead
// of silently losing that opt-in entirely for anyone who used Google.
let _marketingOptInPrompted = false; // guards against onLoggedIn firing more than once in a session
async function maybePromptMarketingOptIn(){
  ...
  const isBrandNew = createdAt && (Date.now() - createdAt) < 60_000; // only right after signup, not on every later login
  if(!isGoogle || alreadyDecided || !isBrandNew) return;
...
function renderTopbar(){
  ...
  // screen) and tappable, since discovering "hey, opening packs earns
  // you a rank with a daily credit bonus" is the whole point of having
  // the system in the first place. Skipped for guests (no rank exists
  // outside a real account) and admins (already shown their own
  // distinct "System Admin" status on Profile — a rank chip here would
  // just be a second, redundant status readout for them specifically).
  let rankChip = '';
  if (session && profile && !profile.is_admin) {
    const { current } = getTierProgress(profile);
    rankChip = `<button class="tappable" id="rank-chip-btn" style="display:flex; align-items:center; gap:6px; padding:6px 12px; border-radius:999px; border:1px solid var(--edge); background:var(--panel); color:${current.color}; font-size:12px; font-weight:700; flex-shrink:0; margin-right:8px;">${tierIconSVG(current.key, 15)}<span>${current.label}</span></button>`;
  }

...
/* ============================================================
   ...
   card reveals. Driven entirely by two CSS custom properties
   (--holo-px/--holo-py, 0-100%) set from JS as the pointer moves
   (desktop) or the device tilts (mobile) — see attachHoloTilt()/
   detachHoloTilt() near vibrate(). Static .shimmer/.legendary-
   shimmer (existing) stay as the base holo look; this layers a
   moving specular highlight on top so it responds to how you're
   actually holding/dragging across the card, not just a canned
   loop.
   ============================================================ */
.face.front.holo-active { position: relative; }
.face.front.holo-active .holo-layer {
  ...
  background-position: var(--holo-px, 50%) var(--holo-py, 50%), var(--holo-px, 50%) var(--holo-py, 50%);
  mix-blend-mode: color-dodge;
}
.face.front.holo-active.holo-live .holo-layer { opacity: 0.9; }
...
function detachHoloTilt(faceFront){
  ...
  (faceFront || document.querySelector('.face.front.holo-active'))?.classList.remove('holo-active', 'holo-live');
}

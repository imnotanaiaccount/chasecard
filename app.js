...
function generatePack(cards, game, packSize){
  ...
  for(let i=0;i<nHits;i++){
    let card;
    if(isGodPack){ const hi = buckets[8]||buckets[7]||buckets[6]||buckets[5]||buckets[4]||buckets[3]; card = hi ? pickFromExcluding(hi, usedIds) : weightedPickExcluding(buckets, HIT_WEIGHTS, usedIds); }
    else card = weightedPickExcluding(buckets, HIT_WEIGHTS, usedIds);

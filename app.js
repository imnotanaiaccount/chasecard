...
async function getJPSets(){
  ...
  try{
    ...
    // comment below), just missed here. getSetsPokemon() awaits this for
    // BOTH the EN and JP tabs (JP sets are appended after EN sets below),
    // so a stalled TCGdex connection here didn't just break the JP tab —
    // it hung getSets() entirely, which is why the global loading bar
    // (tied to in-flight requests) never cleared and no packs ever
    // rendered, for guests and logged-in users alike.
...
async function loadProfile(attempt=1){
  ...
  }
  {
    ...
    if (creditCountEl) creditCountEl.textContent = currentCredits();
    syncAchievementsQuiet();
    const streakSlot = $('#login-streak-slot');
    if (streakSlot && !streakSlot.hasChildNodes()) {
      const streakCard = renderLoginStreakCard(profile);
...
async function renderHome(){
  ...
  // Stable id, always present (even empty) — see refreshLoginStreakSlot()
  // in loadProfile(), which fills this in retroactively if Home renders
  // before profile has loaded (profile is null at that point, so
  // renderLoginStreakCard returns null and there's nothing to show yet).
  const streakSlot = el('div');
  streakSlot.id = 'login-streak-slot';
  const streakCard = renderLoginStreakCard(profile);
  if(streakCard) streakSlot.appendChild(streakCard);
  setsWrap.appendChild(streakSlot);

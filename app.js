...
async function beginOpen(setMeta, packCost, qty = 1){
  ...
  try{
    ...
    if(guestMode){
      ...
      const { data, error } = await sb.rpc('open_pack', {
      ...
      if(data.new_balance != null){
        ...
        const creditCountEl = $('#credit-count');
        if(creditCountEl) creditCountEl.textContent = data.new_balance;
      }
    }
...
async function renderPackContents(setMeta){
  ...
  try{
    ...
    cards.forEach(c => {
      const item = el('div','coll-item');
      const srcUrl = c.images?.large || c.images?.small || '';
      ...
      item.addEventListener('click', async ()=>{
        showCardFullscreen(await ImgCache.get(srcUrl), null);
      });
      grid.appendChild(item);
      if(srcUrl){
        ImgCache.get(srcUrl, true).then(src => {
          const imgEl = item.querySelector('img');
          if(imgEl && src) imgEl.src = src;
        });
      }

...
async function renderSetDetail(setMeta){
  ...
  try{
    ...
    async function isDuplicate(resolvedUrl) {
      let buf;
      try { buf = await fetchImageBytes(resolvedUrl); } catch (e) { return false; } // couldn't read bytes at all — show it, can't prove it's a dup
      const phash = await perceptualHash(buf);
      if (phash) {
        if (shownPHashes.some(h => hammingDistance(h, phash) <= PHASH_DUP_THRESHOLD)) return true;
      ...
      try {
        ...
        if (shownExactHashes.has(hex)) return true;
    ...
    const resolvedInOrder = await Promise.all(rawUrls.map(async (url) => {
      if (shownUrls.has(url)) return null;
      shownUrls.add(url);
      try { return await ImgCache.get(url, true); } catch (e) { return null; }
    }));

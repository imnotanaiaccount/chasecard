...
async function attachHoloTilt(faceFront, isLegendary){
  ...
  // prompt that must be triggered by a user gesture. Asking once, the
  // first time a holo-tier card is actually on screen (not on app
  // load, where a cold prompt gets auto-dismissed/ignored) is the only
  // point in this flow that's both a real gesture and relevant context.
  if(typeof DeviceOrientationEvent !== 'undefined' && typeof DeviceOrientationEvent.requestPermission === 'function'){
    if(!window._holoOrientationPermAsked){
      window._holoOrientationPermAsked = true;
      try{ await DeviceOrientationEvent.requestPermission(); }catch(e){ /* denied — pointer/tap still works */ }
    }
  }
...
const OPTCG_SETS_CACHE_KEY = 'cache_punkrecords_sets_v2';
...
function formatOnePiecePackName(s){
  // BUGFIX: this used to prepend the raw pack `id` whenever it wasn't
  // already inside raw_title. That's fine for alphanumeric ids (OP01,
  // ST01) but punk-records also uses long numeric ids for some packs
  // (e.g. "569001" — see the packs.json schema comment above), and THAT
  // is what was getting prepended: "569001: Starter Deck" on a narrow
  // single-line-ellipsis card means the ugly number is the only thing
  // that survives truncation, and the part that actually distinguishes
  // one starter deck from another (the box art / character name) gets
  // pushed off-screen entirely.
  //
  // title_parts breaks raw_title into its real components — prefix (a
  // short pack code, when one exists), title (category, e.g. "Starter
  // Deck"), and label (the distinguishing bit, e.g. "-Straw Hat Crew-").
  // Build the display name from title+label first, using prefix (never
  // the raw id) as an optional short lead-in. Only fall back to raw_title
  // or the bare id when title_parts has nothing usable at all.
...
// card — see .set-card .name CSS) and "Starter Deck - " is present on every
// starter deck's name, so it's the part of the label LEAST likely to help
// someone tell packs apart there, while eating space the actually-
// distinguishing text (which box/character) needs. Strips that leading
// segment for the grid only; setMeta.name itself is untouched everywhere
// else (pack hero header, pack-count text, share text, etc.) where the
// fuller "Starter Deck - X" wording is still useful context.
function onePieceGridDisplayName(s){
  if (s.series !== 'One Piece' || !s.name) return s.name;
  return s.name.replace(/^starter deck\s*[-\u2013\u2014:]\s*/i, '').trim() || s.name;
}

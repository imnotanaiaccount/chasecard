...
function packCompositionFor(game, packSize, setMeta){
  ...
  if(packSize === 11){
    ...
    // slot at all — see generatePack's noReverseHolo branch, which skips
    // the guaranteed foil placement entirely for those. commons is 6
    // either way (PSA's own count, not the old approximated 5) — the two
    // sub-eras differ only in whether the guaranteed foil slot exists.
    const d = setMeta?.releaseDate ? new Date(setMeta.releaseDate) : null;
    const noReverseHolo = !!(d && !isNaN(d) && d < new Date('2002-05-24'));
    return { commons: 6, uncommons: 3, hitSlots: 1, ygo: false, noReverseHolo };
  }
...
function generatePack(cards, game, packSize, setMeta){
  ...
  if(battlePack){
    ...
    // existed in real packs from this window, so this slot is skipped
    // outright — same treatment as the energy card every era already
    // drops from the tracked count (see realPackSizeFor's note), not
    // backfilled with an extra common. Pack comes out to 10 tracked
    // cards here instead of 11, same as every other era in this file
    // already omits its own non-collectible slot(s).
  } else {
    ...
    if (session && profile?.is_admin) {
        ...
        if (pipelineStatusEl) {
          sb.rpc('admin_get_email_pipeline_status').then(({ data, error }) => {
            ...
            const { resend_key_configured, winback_cron_active } = data[0];
            if (resend_key_configured && winback_cron_active) {
              pipelineStatusEl.innerHTML = `Automated welcome &amp; win-back emails (Resend): <strong style="color:#4ade80;">active</strong>`;
            } else {
              const missing = [];
              if (!resend_key_configured) missing.push('Resend API key not set in Vault');
              if (!winback_cron_active) missing.push('daily win-back cron job disabled');
              pipelineStatusEl.innerHTML = `Automated welcome &amp; win-back emails (Resend): <strong style="color:var(--danger);">not sending</strong> — ${missing.join(', ')}.`;
            }

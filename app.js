...
function renderLoginStreakCard(profile){
  ...
  function renderInner(){
    ...
    const streak = profile.login_streak_current || 0;
    ...
    const todayDayNumber = claimedToday ? Math.max(streak,1) : streak + 1;
      ...
      btn.addEventListener('click', async () => {
        ...
        try{
          ...
          if(data?.claimed){
            ...
            profile.last_streak_claim_date = todayStr;
            profile.credits = data.credits;
            const creditCountEl = $('#credit-count');
            if(creditCountEl) creditCountEl.textContent = currentCredits();
            SFX.coin?.();
            vibrate([15,30,15]);
            toast(`+${data.reward} credits — ${data.streak} day streak!`, 2500);
            // award_achievement no-ops (on conflict do nothing) once
            // already earned, so it's safe to call this every day past
            // the threshold rather than only on the exact day it's first
            // crossed — no risk of double-celebrating or double-paying.
...
async function renderSetDetail(setMeta){
  ...
  wrap.innerHTML = `
    <div class="pack-hero">
      ...
      <div class="set-progress-wrap" id="set-progress-wrap" style="display:none;">
        <div class="set-progress-label">
          <span id="set-progress-text">— / — cards collected</span>
          <span id="set-progress-pct" style="color:var(--dim);"></span>
        </div>
        <div class="set-progress-track"><div class="set-progress-fill" id="set-progress-fill" style="width:0%;"></div></div>
      </div>
  ...
  try{
    ...
    // Completion progress for THIS set — the pack shop page previously
    // showed only static facts (10/pack, N total cards) with no sense of
    // how far along the active collection actually is toward finishing
    // it. A thin labeled bar right under those facts, matching the same
    // visual language as the Rank progress bar elsewhere, answers that at
    // a glance without a trip to the full checklist — tapping it still
    // goes there for the card-by-card view.
    {
      const progressWrap = $('#set-progress-wrap', wrap);
      const jpCards = setMeta.id.startsWith('jp-') ? filterToGenuineArt(cards) : cards;
      if (progressWrap && jpCards.length > 0) {
        const map = getCollectionsMap();
        const coll = map[getActiveCollectionName()] || {};
        const owned = jpCards.filter(c => coll[c.id] && coll[c.id].count > 0).length;
        const pct = Math.round((owned / jpCards.length) * 100);
        progressWrap.style.display = 'block';
        progressWrap.style.cursor = 'pointer';
        $('#set-progress-text', wrap).textContent = `${owned} / ${jpCards.length} cards collected`;
        $('#set-progress-pct', wrap).textContent = `${pct}%`;
        $('#set-progress-fill', wrap).style.width = `${Math.max(owned > 0 ? 3 : 0, pct)}%`;
        if (owned >= jpCards.length) $('#set-progress-fill', wrap).classList.add('done');
        progressWrap.onclick = () => render('set_checklist', { set: setMeta });
      }
    }

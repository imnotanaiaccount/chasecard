...
async function renderSetDetail(setMeta){
  ...
  try{
    ...
    async function isDuplicate(resolvedUrl) {
      let buf;
    ...
    async function appendArtCard(url) {
      if (await isDuplicate(url)) return; // genuine visual duplicate — skip
      if (!firstShown) {
    ...
    for (const p of fetchPromises) {
      const resolved = await p; // fetches already in flight from the map() above — this only waits on THIS one, which is often already done by the time we get here
      if (resolved) await appendArtCard(resolved);
    }

...
const CONFIG = {
  ...
  ECONOMY: {
    ...
    // Renamed from PREMIUM_TIERS: these are earned (packs opened +
    // referrals — see TIER_LADDER below), never purchased, so "Premium"
    // was the wrong word for a free progression ladder. The DB column
    // this reads from (profiles.premium_tier) keeps its existing name
    // below — renaming a live column is a schema change, out of scope
    // here — but every user-facing surface now calls this "Rank"
    // instead. Kept fully separate from `is_premium` (see the
    // account-area/referral copy below), which is the one genuinely
    // admin-granted, non-earned status — that one is worth calling out
    // differently precisely because it ISN'T earned the way these are.
    RANK_TIERS: [
      { key:'free',    label:'Free' },
...
/* ============================================================
   ...
   that tier (matches CONFIG.ECONOMY.RANK_TIERS labels above).
   ============================================================ */
const TIER_LADDER = [
  { key:'free',    label:'Free',    packs:0,    referrals:0,  dailyBonus:0,     color:'#94a3b8' },

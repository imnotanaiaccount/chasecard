...
async function getCardsForSetPokemon(setId){
  ...
  if (setId.startsWith('jp-')) {
    ...
    try{
      ...
      // jpdebug: the success-path alert above only fires when a set
      // resolves cleanly — if every set is failing before that point
      // (network error, TCGdex non-200, timeout), this was the only
      // path actually being hit, silently, with nothing visible to show
      // for it. Surfacing it here too, since devtools/Network tab isn't
      // realistically reachable on mobile Chrome the way it is on desktop.
      if (jpDebugOn) alert(`JP DEBUG — set ${setId} FAILED\n${e && e.message ? e.message : String(e)}`);
      if(cached) { toast('Showing cached cards — live data unavailable'); return cached.data; }
      throw e;
    }
  }

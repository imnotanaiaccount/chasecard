...
const Prewarm = {
  ...
  async _resolvePackArtUrlsUncached(setMeta) {
    ...
    const rawUrls = [...new Set([
      ...own,          // hand-picked own-hosted pack photo, if any — tried first,
                        // but no longer the ONLY candidate (see BUGFIX above)
      ...(skipRedundantPackshots ? [] : ghConfirmed),
      // tcgdexUrl and setMeta.images.logo are a generic set LOGO graphic
      // (the "POKÉMON / TRADING CARD GAME" wordmark), not a photographed
      // pack — TCGdex serves it as a stand-in whenever it has no actual
      // booster-pack photography for a set. Treating it as a peer
      // swipeable candidate meant it showed up ALONGSIDE a real curated
      // photo (e.g. Base Set's `own` Blastoise wrapper) as if it were an
      // equally legitimate "alternate art" choice, when really it's the
      // same "nothing better exists" placeholder as the is-fallback
      // branch below — just reached through the gallery instead of it.
      // Gated behind skipRedundantPackshots (== own has real art) for the
      // same reason ghConfirmed/the raw github guesses already are.
      ...(skipRedundantPackshots ? [] : [tcgdexUrl]),
      ...
      ...(skipRedundantPackshots ? [] : [setMeta.images?.logo || null]),
    ].filter(Boolean))];

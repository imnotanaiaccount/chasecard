...
async function getCardsForSet(setId){
  const key = 'cache_cards_v8_' + setId; // v3->v4: JP rarity; v4->v5: pokemontcg.io image fallback added; v5->v6: fixed it to use an English name instead of raw Japanese text; v6->v7: added curated classic-Trainer-card names; v7->v8: added a dexId-based fallback (Pokédex number, locale-independent) that covers Pokémon cards even on sets with no English release to match against at all
    ...
    try{
      ...
      async function rarityWorker() {
        while (cursor < jaCards.length) {
          ...
          try {
            const cRes = await fetchWithTimeout(`https://api.tcgdex.net/v2/ja/cards/${c.id}`);
            if (cRes.ok) {
          ...
          if (enSetRes.ok) {
            ...
            async function enRarityWorker() {
              while (ecursor < enBriefs.length) {
                ...
                try {
                  const ecRes = await fetchWithTimeout(`https://api.tcgdex.net/v2/en/cards/${ec.id}`);
                  if (ecRes.ok) {
      ...
      const stillNoImage = jaCards.filter(c => {
        if (c.image || enByLocalId[c.localId]) return false;
      ...
      const data = jaCards.map(c => {
        ...
        const pokeFallback = !img ? (pokeImgByName[englishNameFor(c)] || pokeImgByDexId[dexIdByCardId[c.id]]) : null;
        return {
          id: 'jp-' + c.id,
          name: c.name,

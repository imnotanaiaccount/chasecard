...
async function getSharedSetCache(setId) {
  try {
    const { data, error } = await sb.from(SET_CARD_CACHE_TABLE).select('data, updated_at').eq('set_id', setId).maybeSingle();
...
//     using (auth.role() = 'authenticated');
// Guests still read fine (getJpFilterCache below never checks session) —
// they just don't contribute new verdicts; putJpFilterCache() skips the
// write attempt entirely when there's no session, rather than sending
// one RLS will reject.
...
async function getJpFilterCache(setId) {
  try {
    const { data, error } = await sb.from(JP_FILTER_CACHE_TABLE).select('genuine, v, updated_at').eq('set_id', setId).maybeSingle();
    if (error || !data) return null;
    if (Date.now() - new Date(data.updated_at).getTime() > JP_FILTER_CACHE_MAX_AGE_MS) return null;
    if (data.v !== JP_FILTER_VERSION) return null; // written under an older pipeline/threshold — treat as a miss
    return data.genuine;
  } catch (e) {
    return null; // table not set up yet / unreachable — fall through to normal resolution
  }
...
async function renderHome(){
  ...
  try{
    ...
    // Japanese sets specifically: only show ones with a REAL resolved
    // pack image, not the CSS/blank placeholder every other tab falls
    // back to when art can't be found. That means resolving art up
    // front here (with real concurrency) instead of the usual
    // show-skeletons-then-fill-in progressive loading below — a
    // deliberate tradeoff so this tab never lists a set it can't show
    // genuine pack art for, at the cost of a brief pause before this
    // tab's grid appears at all. Deliberately kept BEFORE grid.innerHTML
    // is cleared, so the loading skeletons stay visible through that
    // pause instead of leaving a blank grid while it resolves.
    if (activeHomeTab === 'pkmn_jp') {
      if (jpTabFilteredCache) {
        // Already computed this session — skip straight to rendering
        // instead of re-running the whole resolve+filter pipeline.
        displaySets = jpTabFilteredCache;
      } else {
        Prewarm.start(displaySets);
        const resolved = new Array(displaySets.length).fill(null);
        let jpNext = 0;
        const JP_FILTER_CONCURRENCY = 5;
        async function jpFilterLane() {
          while (jpNext < displaySets.length) {
            const i = jpNext++;
            resolved[i] = await resolveSetImageSrc(displaySets[i]);
          }
        }
        await Promise.all(Array.from({ length: JP_FILTER_CONCURRENCY }, jpFilterLane));
        if (myGen !== homeRenderGen) return;
        displaySets = displaySets
          .map((s, i) => ({ s, src: resolved[i] }))
          .filter(x => x.src)
          .map(x => { x.s._resolvedImgSrc = x.src; return x.s; });

        // Second pass: exclude sets whose CARDS are only English-sourced
        // art (see isSetGenuinelyJapanese/_artSource above) — a much
        // heavier check than the pack-art pass above, since it means
        // fully resolving every card in the set (the same pipeline as
        // actually opening a pack from it), not just checking one image
        // URL. Lower concurrency on purpose: each "task" here is itself a
        // multi-request operation (TCGdex + PokéWallet + pokemontcg.io +
        // per-card rarity fetches internally), so running many of those
        // at once would mean dozens of concurrent requests hitting
        // rate-limited sources simultaneously. Thanks to the shared
        // set_card_cache table this cost is paid once globally per set,
        // not once per user — but the first load of a new/uncached set
        // will feel this pass.
        const genuine = new Array(displaySets.length).fill(false);
        let qNext = 0;
        const JP_QUALITY_CONCURRENCY = 3;
        async function qualityLane() {
          while (qNext < displaySets.length) {
            const i = qNext++;
            // Check the shared verdict cache first — a hit means this
            // exact set has already been through the full card-by-card
            // resolve on some device, ever, and skips straight to the
            // answer instead of re-running it. Only a miss (new set,
            // stale version, or the table being unreachable) falls
            // through to the real check, which then writes its result
            // back so nobody — including this same browser next time —
            // has to pay for it again.
            const cached = await getJpFilterCache(displaySets[i].id);
            if (cached !== null) {
              genuine[i] = cached;
              continue;
            }
            const result = await isSetGenuinelyJapanese(displaySets[i]);
            genuine[i] = result;
            putJpFilterCache(displaySets[i].id, result);
          }
        }
        await Promise.all(Array.from({ length: JP_QUALITY_CONCURRENCY }, qualityLane));
        if (myGen !== homeRenderGen) return;
        displaySets = displaySets.filter((s, i) => genuine[i]);
        jpTabFilteredCache = displaySets;
      }
    ...
    const tasks = displaySets.map(s => async () => {
      ...
      // JP sets already had their art resolved during the pre-filter
      // pass above — that resolved image is WHY this set made it into
  ...
  function boot(){
    ...
    $('#flipcard', screen).addEventListener('click', function(){
      if(this.dataset.done==='1'){
        idx++;
        if(idx >= pack.cards.length){ screen.remove(); flashLayer.remove(); showSummary(setMeta, pack, openingId); return; }

...
function openRevealScreen(setMeta, pack, bgUrl, openingId){
  ...
  function finishRip(wasQuickTap){
    ...
    ripEl.addEventListener('animationend', () => {
    ...
    // The impact beat (flash/shockwave/screen-punch) is purely additive
    // flourish on top of the tear, independent of the pack-art hide/
    // mount sequence above, and cleans up its own elements on its own
    // timer — untouched here. This fires a beat after the tear itself
    // (letting the halves start flying apart first) so it reads as the
    // pack RELEASING something, not simultaneous with the tear.
    if(!reducedMotion()){
      setTimeout(() => {

...
/* ============================================================
   GIVEAWAYS
   Server (Supabase RPCs get_giveaway / list_giveaways / enter_giveaway /
   admin_* / draw_giveaway) is the source of truth for progress, entries,
   draws and prize payout — this only renders what those return. Tasks are
   free, in-app actions (open packs, streak, referrals, set milestones), so
   there is never a purchase behind an entry.
   ============================================================ */
const GA_ERRORS = {
  requirements_not_met: 'Finish the required tasks first.',
...
function gaPoolText(prizes){
  ...
  return cr > 0 ? `${gaCompact(cr)} cr` : 'Special prizes';
}
const GA_ICONS = {
...
async function loadGiveawayBanner(slot, gen){
  try{
    ...
    if(won){
      ...
      html = `<div class="ga-banner-k">GIVEAWAYS · ADMIN</div><div class="ga-banner-t">Manage giveaways</div><div class="ga-banner-s">Create, activate and draw</div>`;
    } else return;
...
async function renderGiveawayList(){
  ...
  wrap.innerHTML = `
    <div class="ga-top"><div class="section-title" style="margin:0;">Giveaways</div>${isAdminUser() ? '<button class="btn btn-gold" id="ga-new" style="padding:8px 14px; font-size:13px;">+ New</button>' : ''}</div>
    <div id="ga-list" class="ga-list"><div class="hint">Loading…</div></div>`;
  app.appendChild(wrap);
  const listEl = $('#ga-list', wrap);
  if(isAdminUser()) $('#ga-new', wrap).addEventListener('click', () => openGiveawayCreateSheet());
  let items;
  ...
  if(!items.length){ listEl.innerHTML = '<div class="empty-state">No giveaways running right now — check back soon.</div>'; return; }
  listEl.innerHTML = '';
  items.forEach(g => {
    const endsMs = Date.parse(g.ends_at);
    const open = g.status === 'active' && endsMs > Date.now();
    const pill = g.status === 'draft' ? ['Draft', 'muted'] : g.status === 'drawn' ? ['Winners drawn', 'muted'] : open ? ['Active', 'live'] : ['Ended', 'muted'];
    ...
    card.innerHTML = `
      ...
      <div class="ga-card-body">
        ...
        <div class="ga-card-row" style="align-items:flex-end; margin-top:14px;">
          ...
          <div style="text-align:right;"><div class="ga-k">Prize pool</div><div class="ga-big">${g.credit_pool > 0 ? gaCompact(g.credit_pool) + ' cr' : `${g.prize_count} prizes`}</div></div>
        </div>
        <button class="btn btn-gold ga-cta" type="button">${g.i_won ? 'You won — view' : (open ? (g.my_entries ? `You're in · ${g.my_entries} ${g.my_entries === 1 ? 'entry' : 'entries'}` : 'Enter Giveaway') : 'View')}</button>
      </div>`;
...
async function renderGiveawayDetail(id){
  ...
  function paint(d){
    ...
    const pill = g.status === 'draft' ? ['Draft', 'muted'] : g.status === 'cancelled' ? ['Cancelled', 'muted'] : g.status === 'drawn' ? ['Winners drawn', 'muted'] : open ? ['Active', 'live'] : ['Ended — drawing soon', 'muted'];

    const prizesHtml = d.prizes.map(p => `
      <div class="ga-prize ${p.place === 1 ? 'top' : ''}">
        <div class="ga-prize-badge">${gaOrdinal(p.place)}</div>
        <div class="ga-prize-art">${GA_ICONS[p.prize_type] || GA_ICONS.custom}</div>
    ...
    if(open && !admin){
      ...
      } else action = `<button class="btn btn-gold ga-enter" id="ga-enter" ${requiredOk ? '' : 'disabled'}>${requiredOk ? 'Enter Giveaway' : 'Complete required tasks to enter'}</button>`;
    ...
    if(g.status === 'drawn'){
      const rows = d.winners.map(w => `
        <div class="ga-winner ${w.is_me ? 'me' : ''}">
          ...
          <div class="ga-winner-main"><b>${escapeHtml(w.username)}${w.is_me ? ' (you!)' : ''}</b><span>${escapeHtml(w.label)}${w.prize_type === 'custom' && !w.granted ? ' · being arranged' : ''}</span></div>
        </div>`).join('');
      const me = d.winners.find(w => w.is_me);
      winnersHtml = `
        <div class="section-title" style="margin:22px 0 10px;">Winners</div>
        ${me ? `<div class="ga-won">You won ${escapeHtml(me.label)}!${me.prize_type === 'custom' ? ' We\'ll be in touch about your prize.' : ' It\'s already in your account.'}</div>` : ''}
        ${rows || '<div class="hint">No eligible entries this time.</div>'}`;
    }

    let adminHtml = '';
    if(admin){
      adminHtml = `
        <div class="ga-admin"><div class="ga-k">Admin</div>
          ${g.status === 'draft' ? '<button class="btn btn-gold" data-act="activate">Activate (starts the clock now)</button>' : ''}
          ${g.status === 'active' ? `<button class="btn btn-gold" data-act="draw">${ended ? 'Draw winners' : 'Draw winners now (early)'}</button>` : ''}
          ${(g.status === 'draft' || g.status === 'active') ? '<button class="btn btn-secondary" data-act="cancel">Cancel giveaway</button>' : ''}
        </div>`;
    }
    ...
    body.innerHTML = `
      ...
      ${reqs.length ? `<div class="section-title" style="margin:22px 0 4px;">Requirements</div>
        <div class="hint" style="margin-bottom:10px;">Everything counts from when the giveaway started. Required tasks unlock entry; bonus tasks add extra entries. All free — no purchase needed.</div>
    ...
    if(enterBtn) enterBtn.addEventListener('click', async () => {
      ...
      try{
        const { data, error } = await sb.rpc('enter_giveaway', { p_id: id });
        if(error) throw error;
        gaListCache = null;
    ...
    body.querySelectorAll('[data-act]').forEach(btn => btn.addEventListener('click', async () => {
      ...
      if(act === 'draw' && !confirm(ended ? 'Draw the winners now? Credits and pack tickets are paid out immediately and this can\'t be undone.' : 'This giveaway hasn\'t ended yet. Draw winners early? Prizes are paid out immediately and this can\'t be undone.')) return;
      if(act === 'cancel' && !confirm('Cancel this giveaway? Entries are kept but no prizes are paid.')) return;
      btn.disabled = true;
      try{
        if(act === 'draw'){
          const { data, error } = await sb.rpc('draw_giveaway', { p_id: id, p_force: !ended });
          if(error) throw error;
          toast(`Drawn — ${data?.winners ?? 0} winner${data?.winners === 1 ? '' : 's'}`, 3000);
        } else {
          const { error } = await sb.rpc('admin_set_giveaway_status', { p_id: id, p_status: act === 'activate' ? 'active' : 'cancelled' });
          if(error) throw error;
          toast(act === 'activate' ? 'Giveaway is live' : 'Giveaway cancelled');
        }
        gaListCache = null;
        repaint();
      }catch(e){
        btn.disabled = false;
        toast(gaErrorMessage(e, 'That didn\'t work — check the console.'), 3200);
...
// Admin quick-create: covers the common shape (credit prizes for 1st-3rd +
// standard tasks). Anything fancier (pack tickets, custom prizes) goes
// through admin_create_giveaway directly. Creates a DRAFT — nothing is
// visible to players until you press Activate.
function openGiveawayCreateSheet(){
  const overlay = el('div', 'overlay');
  const sheet = el('div', 'sheet');
  overlay.appendChild(sheet);
  document.body.appendChild(overlay);
  ...
  sheet.innerHTML = `
    <div class="sheet-handle"></div>
    <h2>New giveaway</h2>
    <div class="sub">Saved as a draft — you activate it when you're ready.</div>
    <label class="ga-f">Title<input id="gc-title" class="auth-form" maxlength="120" placeholder="e.g. Launch Week Giveaway" /></label>
    <label class="ga-f">Runs for
      <select id="gc-days" class="auth-form"><option value="3">3 days</option><option value="7" selected>7 days</option><option value="14">14 days</option><option value="30">30 days</option></select>
    </label>
    <div class="ga-f">Credit prizes (leave blank to skip a place)</div>
    <div class="ga-f-row"><input id="gc-p1" type="number" min="1" class="auth-form" placeholder="1st" /><input id="gc-p2" type="number" min="1" class="auth-form" placeholder="2nd" /><input id="gc-p3" type="number" min="1" class="auth-form" placeholder="3rd" /></div>
    <div class="ga-f">Tasks</div>
    <label class="ga-chk"><input type="checkbox" id="gc-r-packs" checked /> Required: open <input id="gc-r-packs-n" type="number" min="1" value="3" class="auth-form" style="width:64px; display:inline-block; padding:6px 8px;" /> packs</label>
    <label class="ga-chk"><input type="checkbox" id="gc-r-streak" checked /> Bonus +2: reach a 3-day streak</label>
    <label class="ga-chk"><input type="checkbox" id="gc-r-ref" checked /> Bonus +5: refer a friend who joins</label>
    <label class="ga-chk"><input type="checkbox" id="gc-r-ms" checked /> Bonus +3: claim a set milestone reward</label>
    <div id="gc-msg" class="hint" style="min-height:16px; margin-top:8px;"></div>
    <button class="btn btn-primary" id="gc-save" style="width:100%; margin-top:6px;">Create draft</button>`;
  $('#gc-save', sheet).addEventListener('click', async () => {
    const msg = $('#gc-msg', sheet);
    const title = $('#gc-title', sheet).value.trim();
    if(title.length < 3){ msg.textContent = 'Give it a title (3+ characters).'; return; }
    const prizes = [];
    ['gc-p1', 'gc-p2', 'gc-p3'].forEach((idn, i) => {
      const n = parseInt($('#' + idn, sheet).value, 10);
      if(n >= 1) prizes.push({ place: prizes.length + 1, label: ['1st place', '2nd place', '3rd place'][prizes.length], prize_type: 'credits', amount: n });
    });
    if(!prizes.length){ msg.textContent = 'Add at least one credit prize.'; return; }
    const requirements = [];
    if($('#gc-r-packs', sheet).checked){ const n = Math.max(1, parseInt($('#gc-r-packs-n', sheet).value, 10) || 3); requirements.push({ kind: 'open_packs', label: `Open ${n} pack${n === 1 ? '' : 's'}`, target: n, required: true, bonus_entries: 0 }); }
    if($('#gc-r-streak', sheet).checked) requirements.push({ kind: 'login_streak', label: 'Reach a 3-day streak', target: 3, required: false, bonus_entries: 2 });
    if($('#gc-r-ref', sheet).checked) requirements.push({ kind: 'referrals', label: 'Refer a friend who joins', target: 1, required: false, bonus_entries: 5 });
    if($('#gc-r-ms', sheet).checked) requirements.push({ kind: 'set_milestones', label: 'Claim a set milestone reward', target: 1, required: false, bonus_entries: 3 });
    const btn = $('#gc-save', sheet);
    btn.disabled = true;
    try{
      const { error } = await sb.rpc('admin_create_giveaway', { p: { title, days: parseInt($('#gc-days', sheet).value, 10), prizes, requirements } });
      if(error) throw error;
      gaListCache = null;
      overlay.remove();
      toast('Draft created — open it and press Activate');
      render('giveaways');
    }catch(e){
      btn.disabled = false;
      msg.textContent = gaErrorMessage(e, 'Couldn\'t create it — check the console.');
      console.warn('admin_create_giveaway failed', e);
    }
  });
}

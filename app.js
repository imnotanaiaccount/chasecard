...
/* Free-pack-ticket display on a set's pack page. Was a single-line
   colored text strip (easy to skim past) — this is a proper ticket-
   stub visual instead: icon in a circular well, a big count, and a
   dashed tear-line separating the "what" from the "how many", the
   same visual language as an actual redemption ticket. Kept as its
   own component (not reused from anywhere else) since this is the
   one place in the app a free pack is actually redeemable. */
.pack-ticket-card {
  width: 100%;
  margin: 6px 0 2px;
  display: flex;
  ...
  padding: 10px 14px;
  border-radius: 12px;
  background: linear-gradient(135deg, rgba(77,232,224,0.16), rgba(77,232,224,0.04));
  border: 1px solid rgba(77,232,224,0.45);
  box-shadow: 0 0 0 1px rgba(0,0,0,0.06), 0 4px 14px rgba(77,232,224,0.12);
}
.pack-ticket-icon-well {
  flex: 0 0 auto;
  width: 44px;
  height: 44px;
  border-radius: 50%;
  background: rgba(77,232,224,0.14);
  display: flex;
  align-items: center;
  justify-content: center;
}
.pack-ticket-icon-well svg { width: 28px; height: 28px; }
.pack-ticket-divider {
  flex: 0 0 auto;
  align-self: stretch;
  width: 0;
  border-left: 2px dashed rgba(77,232,224,0.4);
  margin: 2px 0;
}
.pack-ticket-text { flex: 1 1 auto; text-align: left; min-width: 0; }
.pack-ticket-count {
  color: #4de8e0;
...
const ImgCache = {
  ...
  // 2026-09-23: mirror bucket had grown to 16GB (42k objects, avg ~400KB,
  // some near the old 5MB/object bucket cap) off raw, un-resized source
  // blobs — blew through the org's storage quota and got requests dropped.
  // This cap is applied to everything written to the SHARED mirror bucket
  // (see the upload() call in _getUncached below), separate from and
  // larger than _THUMB_MAX_DIM above — big enough to still look sharp in
  // the fullscreen card viewer, small enough that the mirror stops growing
  // without bound. Local per-browser tiers (full-res LRU / thumb) are
  // already self-bounding and untouched by this.
  _MIRROR_MAX_DIM: 1000,
  _mirrorDownUntil: 0, // set to a future timestamp on a 402 from Storage — see _getUncached
  ...
  CACHE_NAME: 'chasecards-universal-images-v19', // bumped 2026-09-23: mirror images are now downscaled (see _MIRROR_MAX_DIM) — force a re-fetch instead of serving already-cached full-res blobs from v18
    ...
    try {
      if ('caches' in window) {
        ...
        if (!res) {
          ...
          // 2026-09-23: while the org is over its Supabase storage quota,
          // EVERY request to Storage (mirror reads AND the upload below)
          // comes back 402 instantly — not a timeout, an instant reject.
          // Without this guard every single image load still burns a
          // full 6s waiting on a mirror fetch that can't possibly
          // succeed. _mirrorDownUntil short-circuits straight to origin
          // once we've seen one 402, and re-checks itself automatically
          // once the cooldown passes (e.g. when quota resets) — no
          // redeploy needed either way.
          const mirrorLikelyDown = Date.now() < this._mirrorDownUntil;
          if (!mirrorLikelyDown) {
            res = await this._fetchTimed(mirrorUrl, { mode: 'cors', credentials: 'omit' }, 6000).catch(() => null);
            if (res && res.status === 402) {
              this._mirrorDownUntil = Date.now() + 10 * 60 * 1000; // recheck in 10 min
              res = null;
            }
          }
          if (!res || !res.ok) {
            ...
            if (res && res.ok && Date.now() >= this._mirrorDownUntil) {
              ...
              res.clone().blob().then(async blob => {
                // Downscale+recompress before writing to the SHARED mirror —
                // this bucket is forever-growing (every user, every device,
                // no LRU), unlike the local caches above, so raw multi-MB
                // source scans are what silently blow the storage quota.
                // Falls back to the raw blob only if thumbnailing itself
                // isn't possible in this browser (see _downscale above).
                const mirrorBlob = (await this._downscale(blob, this._MIRROR_MAX_DIM)) || blob;
                return sb.storage.from(POKEWALLET_MIRROR_BUCKET).upload(this._mirrorPath(url), mirrorBlob, {
                  contentType: mirrorBlob.type || blob.type || 'image/webp',
                  upsert: true,
                });
              }).catch(() => {});
            }
...
function generatePack(cards, game, packSize, setMeta){
  ...
  const activeWeights = isModernPokemonTen ? HIT_WEIGHTS_REAL : HIT_WEIGHTS;
  for(let i=0;i<nHits;i++){
    ...
    if(!card && ygo) card = pickFromExcluding(commons, usedIds);
...
function estimatePullOdds(cards, game, packSize, setMeta){
  ...
  const weights = (isPokemon && packSize === 10) ? HIT_WEIGHTS_REAL : HIT_WEIGHTS;
  ...
  for(let i = 0; i < comp.hitSlots; i++){
    ...
    if(!tiersAvail.length){ if(comp.ygo) spread(commons, 1); continue; }
...
function formatOneIn(p){
  const n = 1 / p;
  if(n >= 1000) return `1 in ${(n / 1000).toFixed(1).replace(/\.0$/, '')}k`;
...
function renderChaseStrip(wrap, cards, setMeta, game, coll){
  ...
  host.innerHTML = `
    <div class="chase-head">
      ...
      <div class="chase-sub">${ownedN} of ${picks.length} owned · odds per pack</div>
    </div>
  ...
  picks.forEach(({ card: c, p }) => {
    ...
    tile.setAttribute('aria-label', `${c.name}, ${formatOneIn(p)} packs, ${owned ? 'owned' : 'missing'}`);
    tile.innerHTML = `
      <div class="chase-art skeleton"><img alt="" style="opacity:0;"/></div>
      <div class="chase-name">${escapeHtml(c.name)}</div>
      <div class="chase-odds">${formatOneIn(p)}</div>
...
async function renderSetDetail(setMeta){
  ...
  wrap.innerHTML = `
    <div class="pack-hero">
      ...
      ${(() => {
        if (isAdminUser()) return '';
        const n = myPackTickets[`${ACTIVE_GAME}:${setMeta.id}`] || 0;
        if (n <= 0) return '';
        return `<div class="pack-ticket-card">
          <div class="pack-ticket-icon-well">${GA_ICONS.pack_ticket}</div>
          <div class="pack-ticket-divider"></div>
          <div class="pack-ticket-text">
            <div class="pack-ticket-count">${n} Free Pack${n > 1 ? 's' : ''} Available</div>
            <div class="pack-ticket-sub">for ${escapeHtml(setMeta.name)} — applied automatically, no charge</div>
          </div>
        </div>`;
      })()}

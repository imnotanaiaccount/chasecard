...
const ACHIEVEMENT_META = {
  first_pull:    { icon: '🎴', label: 'First Pull' },
  pack_rat:      { icon: '📦', label: 'Pack Rat' },
  century_club:  { icon: '💯', label: 'Century Club' },
  first_trade:   { icon: '🤝', label: 'First Trade' },
  dealmaker:     { icon: '🔁', label: 'Dealmaker' },
  duelist:       { icon: '⚔️', label: 'Duelist' },
  win_streak_5:  { icon: '🔥', label: 'Win Streak' },
  dealer:        { icon: '💰', label: 'Dealer' },
  high_roller:   { icon: '📈', label: 'High Roller' },
  set_complete:  { icon: '✅', label: 'Set Complete' },
  collector:     { icon: '🗂️', label: 'Collector' },
  whale_watcher: { icon: '💎', label: 'Whale Watcher' },
...
function getAccountTypeBadge(user, userProfile) {
  ...
  switch (tier) {
    ...
    case 'plus': return { label: '🥇 Plus Member', cssClass: 'badge-gold' };
    case 'starter': return { label: '🥉 Starter Member', cssClass: 'badge-bronze' };
...
function renderAccountArea(user, userProfile) {
  ...
  const accountHtml = `
    <div class="account-card">
      <div class="account-header">
        <h3>${user ? escapeHtml(user.email) : 'Guest Session'}</h3>
        <span class="account-badge ${badge.cssClass}">${badge.label}</span>
        ${user ? `<button id="tier-info-toggle" style="background:none; border:1px solid var(--edge); color:var(--dim); border-radius:50%; width:20px; height:20px; font-size:11px; cursor:pointer; margin-left:6px;" aria-label="What are membership tiers?">?</button>` : ''}
      </div>
      ${user ? `
      <div id="tier-info-panel" style="display:none; margin:8px 0 4px; padding:12px; background:var(--panel-2, #1e293b); border:1px solid var(--edge); border-radius:10px; font-size:12.5px; line-height:1.5; color:var(--dim);">
        <strong style="color:var(--text);">What are membership tiers?</strong>
        <p style="margin:6px 0 0;">Tiers aren't for sale — there's no purchase path for them anywhere in this app. You earn your way up automatically by opening packs and referring friends, and the developer can also grant tiers by hand to active community members. Each tier gives a credit bonus every single day on top of whatever you earn from packs, selling, and referrals:</p>
        <ul style="margin:8px 0 0; padding-left:18px;">
          <li>🥉 Starter — 10 packs opened or 1 referral — 500 credits/day</li>
          <li>🥇 Plus — 50 packs or 3 referrals — 1,000 credits/day</li>
          <li>🔥 Pro — 150 packs or 7 referrals — 2,000 credits/day</li>
          <li>💎 Elite — 400 packs or 15 referrals — 4,000 credits/day</li>
          <li>👑 VIP — 1,000 packs or 30 referrals — 10,000 credits/day</li>
        </ul>
        <p style="margin:8px 0 0;">Whichever comes first counts — referrals move you up faster per action than packs do. Tiers only ever go up, never down. Everyone starts on Free — that's normal, not a downgrade. Credits here are virtual only and have no real-world cash value.</p>
      </div>
      ` : ''}
      ${userProfile?.founding_member ? `
      <div style="margin:10px 0; padding:8px 12px; background:linear-gradient(135deg, rgba(255,215,0,0.15), rgba(255,215,0,0.05)); border:1px solid var(--vip-gold-dim); border-radius:10px; display:flex; align-items:center; gap:8px;">
        <span style="font-size:16px;">🏆</span>
        <span style="font-size:12.5px; color:var(--vip-gold); font-weight:700;">Founding Member #${userProfile.founding_member_number} — 2x earned credits, forever</span>
      </div>
      ` : ''}
      <div class="account-details">
        ...
        ${user && myAchievements.length ? `
        <div style="margin-top:10px;">
          <strong>Achievements:</strong>
          <div style="display:flex; flex-wrap:wrap; gap:6px; margin-top:6px;">
            ${myAchievements.map(key => {
              const meta = ACHIEVEMENT_META[key];
              if(!meta) return '';
              return `<span class="account-badge" style="background:var(--panel-2, #1e293b);" title="${escapeHtml(meta.label)}">${meta.icon} ${escapeHtml(meta.label)}</span>`;
            }).join('')}
          </div>
        </div>
        ` : ''}
  ...
  const tierInfoToggle = document.getElementById('tier-info-toggle');
  const tierInfoPanel = document.getElementById('tier-info-panel');
  if(tierInfoToggle && tierInfoPanel){
    tierInfoToggle.addEventListener('click', () => {
      const showing = tierInfoPanel.style.display !== 'none';
      tierInfoPanel.style.display = showing ? 'none' : 'block';
    });
  }
}

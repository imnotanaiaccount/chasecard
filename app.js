...
async function getSetsOnePiece(){
  ...
  try{
    ...
    const list = await res.json();
    if(!Array.isArray(list) || !list.length) throw new Error('punk-records packs.json empty');
...
async function getCardsForSetOnePiece(setId){
  ...
  try{
    // Punk Records is already split one JSON file per pack, so this
    // is a single direct fetch — no full-dump fallback needed like
    // the old optcgapi.com adapter required.
    const res = await fetchWithTimeout(`${PUNKRECORDS_BASE}/cards/${encodeURIComponent(setId)}.json`, 15000);
    if(!res.ok) throw new Error('punk-records cards/' + setId + '.json ' + res.status);
...
function renderBadgesSection(userProfile) {
  const tiles = [];
  // Admin accounts show the Founding badge exactly like anyone else —
  // only when founding_member is genuinely true on the row (e.g. the
  // account was one of the first 1,000 created). No admin override:
  // an admin should only display badges they actually, rightfully
  // earned, same as every other account.

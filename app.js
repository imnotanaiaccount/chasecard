...
/* ============================================================
   ...
   ============================================================ */
// Sheet-based replacements for the native prompt()/confirm() dialogs the
// collection menu used to fall back on — those look and feel jarring next
// to the rest of the app's custom overlay/sheet modals (dust commons,
// card fullscreen, set checklist, etc). Same visual language, reused for
// any "type a name" or "confirm a destructive action" moment.
function openTextInputSheet({ title, sub, placeholder='', defaultValue='', confirmLabel='Confirm', onConfirm }){
  const overlay = el('div','overlay');
  ...
  sheet.innerHTML = `
    ...
    <input type="text" id="text-input-sheet-field" class="auth-form" placeholder="${escapeHtml(placeholder)}" value="${escapeHtml(defaultValue)}" style="width:100%; margin:16px 0 14px;"/>
    <button class="btn btn-primary" id="text-input-sheet-confirm" style="width:100%;">${escapeHtml(confirmLabel)}</button>
  `;

  const input = $('#text-input-sheet-field', sheet);
  ...
  $('#text-input-sheet-confirm', sheet).addEventListener('click', submit);
}

function openConfirmSheet({ title, sub, confirmLabel='Confirm', danger=false, onConfirm }){
  const overlay = el('div','overlay');
  const sheet = el('div','sheet');
  overlay.appendChild(sheet);
  document.body.appendChild(overlay);
  ...
  sheet.innerHTML = `
    ...
    <div style="display:flex; gap:10px; margin-top:16px;">
      <button class="btn btn-secondary" id="confirm-sheet-cancel" style="flex:1;">Cancel</button>
      <button class="btn ${danger ? 'btn-secondary' : 'btn-primary'}" id="confirm-sheet-ok" style="flex:1; ${danger ? 'background:var(--danger); border-color:var(--danger); color:#fff;' : ''}">${escapeHtml(confirmLabel)}</button>
    </div>
  `;

  $('#confirm-sheet-cancel', sheet).addEventListener('click', () => overlay.remove());
  $('#confirm-sheet-ok', sheet).addEventListener('click', () => { overlay.remove(); onConfirm(); });
}

// Generalizes Dust Commons beyond tier 0: sells every EXTRA copy beyond
// 1-of-each, but only for commons/uncommons/rares (tier id <= 2) — the
// same tier-3-and-up cutoff already used for the grid's rare glow and
// the "rare+" summary count, so Holo Rare and up can never get swept up
// in a bulk sale by mistake. Dust Commons stays as its own separate
// action (targets a specific total, tier 0 only); this targets "all my
// duplicates" regardless of which of those three tiers they're in.
const SELL_DUPLICATES_MAX_TIER_ID = 2;
const SELL_DUPLICATES_KEEP_COUNT = 1;

function openSellDuplicatesSheet(coll, activeName){
  const eligible = Object.entries(coll)
    .filter(([, c]) => classifyForCard(c).id <= SELL_DUPLICATES_MAX_TIER_ID && c.count > SELL_DUPLICATES_KEEP_COUNT)
    .map(([id, c]) => ({ id, extra: c.count - SELL_DUPLICATES_KEEP_COUNT, value: getCardSellValue(c) }));
  if(!eligible.length){ toast('No duplicates to sell (commons through rares, beyond 1 copy each)'); return; }

  const totalExtra = eligible.reduce((s,e) => s + e.extra, 0);
  const totalValue = eligible.reduce((s,e) => s + e.extra * e.value, 0);
  ...
  sheet.innerHTML = `
    <div class="sheet-handle"></div>
    <h2>🔁 Sell Duplicates</h2>
    <div class="sub">Keeps 1 of every card, sells the rest — commons through rares only, Holo Rare and up are never touched. ${eligible.length} card${eligible.length===1?'':'s'} have extras: ${totalExtra} copy${totalExtra===1?'':'ies'}, worth ≈${totalValue} credits.</div>
    <button class="btn btn-primary" id="sell-dupes-confirm-btn" style="width:100%; margin-top:16px;">Sell ${totalExtra} Duplicate${totalExtra===1?'':'s'}</button>
  `;

  $('#sell-dupes-confirm-btn', sheet).addEventListener('click', async (e) => {
    e.target.disabled = true; e.target.textContent = 'Selling…';
    overlay.remove();
    await sellDuplicates(activeName);
  });
}

async function sellDuplicates(activeName){
  const map = getCollectionsMap();
  const coll = map[activeName] || {};
  const eligibleIds = Object.keys(coll).filter(id => classifyForCard(coll[id]).id <= SELL_DUPLICATES_MAX_TIER_ID && coll[id].count > SELL_DUPLICATES_KEEP_COUNT);
  if(!eligibleIds.length){ toast('No duplicates to sell'); return; }
  ...
  for(const id of eligibleIds){
    const entry = coll[id];
    if(!entry) continue;
    const excess = entry.count - SELL_DUPLICATES_KEEP_COUNT;
    if(excess <= 0) continue;
    const sellEach = getCardSellValue(entry);

    let actuallySold = 0;
    if(guestMode){
      earned += sellEach * excess;
      actuallySold = excess;
    } else {
      // Same sequential one-at-a-time sell_card pattern as dustCommons —
      // keeps ownership/balance consistent even mid-batch, and stops
      // early on this id (not the whole run) if a sale fails.
  ...
  if(sold){
    SFX.coin();
    toast(`Sold ${sold} duplicate${sold===1?'':'s'} for +${earned} credits!`);
  } else {
    toast('Could not sell duplicates — try again.');
  }
  render(route.name, route.params);
...
/* ============================================================
   Collections View
   ============================================================ */
const COLLECTION_GAME_LABELS = { pokemon: 'Pokémon', onepiece: 'One Piece' };
...
function renderCollection(){
  ...
  // Only worth a filter row once a collection actually spans more than
  // one game — most collectors' vaults are still Pokémon-only today,
  // and an always-there "All / Pokémon" row with a single real option
  // would just be clutter.
  const gamesPresent = [...new Set(keys.map(id => coll[id].game || 'pokemon'))];
  ...
  wrap.innerHTML = `
    <div style="display:flex; flex-direction:column; gap:8px; margin-bottom:14px;">
      <div style="display:flex; gap:8px; align-items:center;">
        ...
        <div style="position:relative; flex-shrink:0;">
          ...
          <div id="coll-menu" style="display:none; position:absolute; right:0; top:calc(100% + 4px); z-index:20; background:var(--panel); border:1px solid var(--edge); border-radius:10px; min-width:190px; overflow:hidden; box-shadow:0 8px 24px rgba(0,0,0,0.35);">
            ...
            <button class="coll-menu-item" id="sell-dupes-btn" style="display:block; width:100%; text-align:left; padding:10px 14px; font-size:13px; background:none; border:none; color:var(--text); cursor:pointer;">🔁 Sell Duplicates</button>
            ...
            <div class="hint" style="padding:0 14px 8px; font-size:10.5px; line-height:1.4;">Changes already save automatically — use this only to force-repair your cloud copy after an import or if a sync ever looks off.</div>
      ...
      ${keys.length ? `<button class="btn btn-secondary" id="open-set-completion-btn" style="width:100%; padding:9px; font-size:12.5px;">📊 Set Completion Overview</button>` : ''}
      ${keys.length ? `<div class="hint" style="font-size:11px; margin:0;">${totalCards} card${totalCards===1?'':'s'} · ${keys.length} unique${(() => { const rareCount = keys.filter(id => classifyForCard(coll[id]).id >= 3).reduce((s,id)=>s+coll[id].count,0); return rareCount ? ` · ${rareCount} rare+` : ''; })()} — tap a card to view or sell it back</div>` : ''}
      ${gamesPresent.length > 1 ? `
      <div id="coll-game-filter" style="display:flex; gap:6px; flex-wrap:wrap;">
        <button class="chip active" data-game="all" style="padding:6px 12px; border-radius:999px; font-size:12px; border:1px solid var(--edge); background:var(--accent, #6366f1); color:#fff; cursor:pointer;">All</button>
        ${gamesPresent.map(g => `<button class="chip" data-game="${g}" style="padding:6px 12px; border-radius:999px; font-size:12px; border:1px solid var(--edge); background:var(--panel); color:var(--text); cursor:pointer;">${escapeHtml(COLLECTION_GAME_LABELS[g] || g)}</button>`).join('')}
      </div>
      ` : ''}
      ${keys.length ? `
  ...
  $('#rename-collection-btn', wrap).addEventListener('click', () => {
    ...
    openTextInputSheet({
      title: '✏️ Rename Collection',
      sub: `Renaming "${activeName}"`,
      defaultValue: activeName,
      confirmLabel: 'Rename',
      onConfirm: (trimmed) => {
        if(trimmed === activeName) return;
  ...
  $('#clear-coll-btn', wrap).addEventListener('click', () => {
    ...
    openConfirmSheet({
      title: '🧹 Clear Cards',
      sub: `This removes every card from "${activeName}". This can't be undone.`,
  ...
  $('#sell-dupes-btn', wrap).addEventListener('click', () => openSellDuplicatesSheet(coll, activeName));
  ...
  $('#delete-collection-btn', wrap).addEventListener('click', () => {
    ...
    openConfirmSheet({
      ...
      sub: `Permanently delete "${activeName}" and all its cards. This can't be undone.`,
      confirmLabel: 'Delete',
      danger: true,
      onConfirm: () => {
        delete map[activeName];
  ...
  const setCompletionBtn = $('#open-set-completion-btn', wrap);
  if(setCompletionBtn) setCompletionBtn.addEventListener('click', () => openSetCompletionSheet(coll, activeName));
  ...
  if(keys.length){
    ...
    let activeGameFilter = 'all'; // one-off per visit, like search — not worth persisting across sessions
    ...
    function appendCardItem(id){
      ...
      // Rare cards get a tier-colored frame + glow, same --tier-color
      // language the tier-progress ladder already uses elsewhere in the
      // app — reserved for Holo Rare and up (tier id 3+) so commons and
      // uncommons, which make up the bulk of most vaults, stay visually
      // quiet and the good pulls actually stand out in the grid.
      const tier = classifyForCard(c);
      if(tier.id >= 3){
        item.style.setProperty('--tier-color', tier.color);
        item.style.border = '1px solid color-mix(in srgb, var(--tier-color) 55%, transparent)';
        item.style.boxShadow = '0 0 10px color-mix(in srgb, var(--tier-color) 35%, transparent)';
      }
    ...
    function drawGrid(){
      ...
      const gameFiltered = activeGameFilter === 'all'
        ? keys
        : keys.filter(id => (coll[id].game || 'pokemon') === activeGameFilter);
      const filteredKeys = query
        ? gameFiltered.filter(id => (coll[id].name || '').toLowerCase().includes(query))
        : gameFiltered;
    ...
    const gameFilterRow = $('#coll-game-filter', wrap);
    if(gameFilterRow){
      gameFilterRow.addEventListener('click', (e) => {
        const btn = e.target.closest('.chip');
        if(!btn) return;
        activeGameFilter = btn.dataset.game;
        gameFilterRow.querySelectorAll('.chip').forEach(c => {
          const isActive = c === btn;
          c.classList.toggle('active', isActive);
          c.style.background = isActive ? 'var(--accent, #6366f1)' : 'var(--panel)';
          c.style.color = isActive ? '#fff' : 'var(--text)';
        });
        drawGrid();
      });
    }
...
/* ============================================================
   ...
   ============================================================ */
// Set-level completion, computed from card IDs alone — every card ID in
// this app follows a reliable `{setId}-{cardNumber}` pattern (confirmed
// against real set_card_cache/collection_cards data across Pokémon EN,
// JP, and One Piece), so set membership never needs a schema change or
// a stored setId. Only fetches the sets the collector actually owns
// cards from — not every set in the game — so this stays fast and cheap
// even though getCardsForSet() ultimately reads from set_card_cache,
// which is shared across all users and already warm for any set someone
// has actually pulled from.
function openSetCompletionSheet(coll, activeName){
  const setIds = new Set();
  const gameForSet = {};
  Object.keys(coll).forEach(id => {
    const cut = id.lastIndexOf('-');
    if(cut <= 0) return; // no derivable set from a hyphen-less/legacy id — skip rather than guess
    const setId = id.slice(0, cut);
    setIds.add(setId);
    if(!gameForSet[setId]) gameForSet[setId] = coll[id].game || 'pokemon';
  });
  if(!setIds.size){ toast('No sets to show yet — open some packs first'); return; }
  ...
  sheet.innerHTML = `
    ...
    <div class="sub">Progress across every set you own at least one card from, in "${escapeHtml(activeName)}".</div>
    <div id="set-completion-list" style="display:flex; flex-direction:column; gap:8px; max-height:55vh; overflow-y:auto; margin-top:14px;"><div class="hint">Loading…</div></div>
  `;
  const listWrap = $('#set-completion-list', sheet);

  (async () => {
    const results = [];
    await Promise.all([...setIds].map(async (setId) => {
      try{
        const game = gameForSet[setId];
        const raw = await getCardsForSet(setId, game);
        // Same JP genuine-art filter renderSetChecklist applies, so this
        // overview's totals always match what tapping into a set shows.
        const cards = setId.startsWith('jp-') ? filterToGenuineArt(raw) : raw;
        if(!cards.length) return;
        const owned = cards.filter(c => coll[c.id] && coll[c.id].count > 0).length;
        results.push({ setId, game, name: cards[0]?.set?.name || setId, owned, total: cards.length });
      } catch(e){
        // Leave a set out entirely if it fails to load — an incomplete
        // overview beats one showing a wrong/zero row for it.
      }
    }));

    if(!results.length){ listWrap.innerHTML = '<div class="hint">Couldn\'t load set data — try again.</div>'; return; }
    results.sort((a,b) => (b.owned/b.total) - (a.owned/a.total));
    ...
    results.forEach(r => {
      ...
      row.innerHTML = `
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
          <div style="font-weight:bold; font-size:13.5px;">${escapeHtml(r.name)}</div>
          <div class="hint" style="margin:0;">${r.owned}/${r.total}</div>
        </div>
        <div style="height:6px; border-radius:999px; background:var(--panel-2, #1e293b); overflow:hidden;">
          <div style="height:100%; width:${pct}%; background:${pct===100 ? 'var(--gold, #facc15)' : 'var(--accent, #6366f1)'};"></div>
        </div>
      `;
      row.addEventListener('click', () => {
        overlay.remove();
        render('set_checklist', { set: { id: r.setId, name: r.name, _game: r.game } });
      });

...
function openGetCreditsModal(lowBalance=false){
  ...
  sheet.innerHTML = `
    ...
    <div class="bundle" style="flex-direction:column;align-items:stretch;gap:8px; margin-top:12px;">
      <div>
        <div class="amt" style="display:flex; align-items:center; gap:6px; ${currentTier?.key==='vip' ? 'color:var(--vip-gold);' : 'color:var(--gold);'}">${currentTier?.key==='vip' ? `<span style="display:inline-flex;">${tierIconSVG('vip', 18)}</span>` : ''}${currentTier?.label || 'Premium'} status</div>

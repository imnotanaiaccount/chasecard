...
function packCompositionFor(game, packSize){
  if(game === 'yugioh'){
    ...
    if(packSize === 5) return { commons: 3, uncommons: 0, hitSlots: 1, ygo: true }; // Duelist Pack / Battle Pack
    return { commons: 7, uncommons: 0, hitSlots: 1, ygo: true }; // mainline 9-card booster
  }
  if(game === 'onepiece'){
    // Real per-pack rarity breakdown isn't published at this granularity
    // either -- approximated proportionally from the default 10-card
    // split below, same caveat as the Yu-Gi-Oh approximations above.
...
function packStructureLabel(game, packSize){
  const c = packCompositionFor(game, packSize);
  if(c.ygo){
...
function generatePack(cards, game, packSize){
  game = game || ACTIVE_GAME;
  packSize = packSize || realPackSizeFor(null, game);
  const { commons: nCommons, uncommons: nUncommons, hitSlots: nHits, ygo } = packCompositionFor(game, packSize);
...
async function renderSetDetail(setMeta){
  ...
  const realStructure = packStructureLabel(setMeta._game || ACTIVE_GAME, realPackSize);
...
async function beginOpen(setMeta, packCost, qty = 1){
  ...
  try{
    ...
    if(guestMode){
      ...
      for(let i=0; i<qty; i++) openedPacks.push(generatePack(cardsForPack, ACTIVE_GAME, packSize));
    } else {

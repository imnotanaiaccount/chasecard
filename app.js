...
// Shockwave rings — 1-3 expanding, glowing rings drawn from a point. Used
...
// confetti falling — the ring reads instantly even in someone's peripheral
// vision, which individual particles don't.
let rings = [];
function burstShockRing(count=2, colors=['#bff6ff','#4de8e0'], originY=innerHeight*0.42){
  if(!cctx || !confettiCanvas || reducedMotion()) return;
  for(let i=0;i<count;i++){
    rings.push({ x: innerWidth/2, y: originY, r: 6, maxR: 260 + i*90, life: 1, speed: 11 - i*2.5, color: colors[i%colors.length], width: 4 - i*0.8 });
  }
  ensureFXLoop();
}
...
// paper chip drifting off-screen. Paired with burstShockRing() at the tear
// impact moment; kept as its own function since other reveal beats may
// want the ring or the shards independently later.
let shards = [];
...
function tickFX(){
  ...
  rings.forEach(r=>{ r.r += r.speed; r.speed *= 0.94; r.life = Math.max(0, 1 - r.r/r.maxR);
    cctx.save(); cctx.globalAlpha = r.life*0.8; cctx.strokeStyle = r.color; cctx.lineWidth = r.width;
    cctx.shadowColor = r.color; cctx.shadowBlur = 18;
    cctx.beginPath(); cctx.arc(r.x, r.y, r.r, 0, Math.PI*2); cctx.stroke(); cctx.restore();
  });
  ...
  if(particles.length || sparks.length || shards.length || rings.length){ requestAnimationFrame(tickFX); } else { fxRunning = false; }
}
  ...
  function finishRip(wasQuickTap){
    ...
    if(!reducedMotion()){
      setTimeout(() => {
        ...
        burstShockRing(2, ['#bff6ff','#4de8e0'], innerHeight*0.42);

...
async function renderHome(){
  ...
  const scrollHint = el('div','tab-scroll-hint'); scrollHint.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>';
  tabScrollWrap.append(tabsWrap, fadeLeft, fadeRight, scrollHint);
  setsWrap.appendChild(tabScrollWrap);
  // Reflects actual scroll position — fades/hint only show on whichever
  // edge genuinely still has more tabs to reveal, and disappear for good
  // once you've scrolled that far, rather than a static decoration that
  // stays up even after you've already found everything.
  function updateTabScrollAffordance(){
    const maxScroll = tabsWrap.scrollWidth - tabsWrap.clientWidth;
    const canScroll = maxScroll > 4;
    fadeLeft.classList.toggle('visible', canScroll && tabsWrap.scrollLeft > 4);
    const atEnd = tabsWrap.scrollLeft >= maxScroll - 4;
    fadeRight.classList.toggle('visible', canScroll && !atEnd);
    scrollHint.classList.toggle('visible', canScroll && !atEnd);
  }

...
function openRevealScreen(setMeta, pack, bgUrl, openingId){
  ...
  const packBgStyle = authenticPackBg ? `background-image:${authenticPackBg}; background-size:100% 100%;` : 'background:linear-gradient(135deg, #1e293b, #0f172a);';
  ...
  intro.innerHTML = `
    <div class="pack-art idle-pulse ${setMeta.resolvedPackArt ? '' : 'is-fallback'}" id="rip-wrapper" style="margin:0; cursor:grab; touch-action:none;">
      <div class="pack-art-bg" style="${packBgStyle}"></div>
      <div class="pack-crimp top fallback-only"></div>
      <div class="pack-crimp bottom fallback-only"></div>
  ...
  function boot(){
    ...
    function goNext(){
      ...
      idx++;
      const seen = revealed.has(idx);
      if(seen) showCard(idx, { instant: true });
      else setTimeout(()=>showCard(idx), 180);
    }
    function goPrev(){
      if(revealing) return;
      if(idx === 0) return;
...
function renderCollection(){
  ...
  wrap.innerHTML = `
    <div style="display:flex; flex-direction:column; gap:12px; margin-bottom:16px;">
      ...
      <div class="hint" style="font-size:11px;">💡 Tap any card in your collection to view details or sell it back for credits — rarer cards are worth more.</div>
    </div>

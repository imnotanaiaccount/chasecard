...
function formatOnePiecePackName(s){
  const id = s.id || '';
  const descriptive = (s.raw_title || s.title_parts?.title || '').trim();
  if (!descriptive) return id;
  return descriptive.toUpperCase().includes(String(id).toUpperCase())
    ? descriptive
    : `${id}: ${descriptive}`;
}
...
async function renderSetDetail(setMeta){
  ...
  try{
    ...
    const shownHashes = new Set();     // content hashes of images actually rendered — see BUGFIX below

    // BUGFIX: dedup used to key off `url` = the *resolved* blob: URL from
    // ImgCache.get(). createObjectURL() mints a brand-new blob: URL on every
    // call, even for byte-identical content, so that check could never catch
    // two different raw sources (own art / GitHub packshots / niche art /
    // the raw.githubusercontent guesses) that just happen to serve the exact
    // same photo — which is common for popular sets. The result: the same
    // pack art appeared as 2-3 "different" swipeable options. Now each
    // resolved image's actual bytes are hashed (SHA-256) and THAT is what's
    // deduped on, so true visual duplicates collapse into one entry
    // regardless of which source they came from.
    async function contentHash(resolvedUrl) {
      try {
        if (!window.crypto?.subtle) return null; // no SubtleCrypto (non-secure context) — skip, fall back to url-only dedupe
        // BUGFIX: ImgCache.get() normally returns a blob: URL, which fetch()
        // reads locally with no CORS involved. But its last-resort path
        // (Supabase mirror miss AND the CDN_IMAGE_BASE Worker proxy both
        ...
        // (opaque/CORS), so contentHash silently returned null and dedup
        // was skipped entirely for exactly the sources most likely to
        // duplicate another candidate (English pack art most of all — it
        // has the most niche/CORS-unfriendly sources). Route anything that
        // isn't already a blob: URL through the same Worker proxy
        // ImgCache uses, purely to get hashable bytes.
        ...
        const buf = await (await fetch(fetchUrl)).arrayBuffer();
    ...
    async function appendArtCard(url) {
      ...
      if (hash) {
        if (shownHashes.has(hash)) return; // genuine visual duplicate — skip

...
// TEMP DEBUG (remove once the "English cards in JP packs" issue is
// confirmed/fixed): records what the last PokéWallet set-lookup actually
// returned, since there's no devtools access on mobile to read the
// Network tab directly. Surfaced via an alert() when ?jpdebug=1 is in
// the URL — see the alert call in getCardsForSet() below.
let _lastPokeWalletSetDebug = null;
async function pokeWalletFetch(path) {
  if (!CONFIG.POKEWALLET_API_KEY) { _lastPokeWalletSetDebug = { path, note: 'no API key configured' }; return null; }
  const ctrl = new AbortController();
  ...
  try {
    ...
    if (!res.ok) { _lastPokeWalletSetDebug = { path, status: res.status, ok: false }; return null; }
...
async function getCardsForSet(setId){
  const key = 'cache_cards_v12_' + setId; // v3->v4: JP rarity; v4->v5: pokemontcg.io image fallback; v5->v6: fixed fallback to use English name; v6->v7: curated Trainer names; v7->v8: dexId-based fallback; v8->v9: empty-result handling fix; v9->v10: added PokéWallet native-JP-art fallback; v10->v11: fixed fallback order so PokéWallet actually runs instead of being pre-empted by the English TCGdex asset; v11->v12: fixed PokéWallet card_number field (was reading the wrong path, so it never matched anything)
  // TEMP DEBUG: ?jpdebug=1 forces a live refetch so the diagnostic below
  // actually runs, instead of silently returning a stale 7-day cache
  // entry from before this fix (or from an earlier debugging attempt).
  ...
  if (setId.startsWith('jp-')) {
    ...
    try{
      ...
      // this is resolved) — opt-in via ?jpdebug=1 so it never shows for
      // anyone not actively debugging.
      if (jpDebugOn) {
        alert(
          `JP DEBUG — set ${realId}\n` +
          ...
          `No image at all: ${dbgNone}\n\n` +
          `PokéWallet set lookup: ${pokeWalletAttempted ? '' : '(skipped — every card already had native ja TCGdex art, so PokéWallet was never called)'}\n${pokeWalletAttempted ? JSON.stringify(_lastPokeWalletSetDebug, null, 2) : ''}`
        );
      }

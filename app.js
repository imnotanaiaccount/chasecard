...
function openRevealScreen(setMeta, pack, bgUrl, openingId){
  ...
  function finishRip(wasQuickTap){
    ...
    // Belt-and-suspenders: don't rely solely on animation-fill-mode:forwards
    // holding ripEl at opacity 0 across the async gap until intro.remove()
    // fires at 620ms below. Nested animations on different elements (this
    // inline one on ripEl vs the class-based fly-top/fly-bottom ones on its
    // pack-half children) can occasionally decouple for a frame during
    // compositing, especially on lower-power devices — reading as the pack
    // art quickly flickering back into view right before the reveal screen
    // mounts. Forcing it explicitly once the packrip animation has
    // definitely finished (400ms) closes that gap.
    setTimeout(() => {
      ripEl.style.opacity = '0';
      ripEl.style.visibility = 'hidden';
    }, 420);
    // The two halves (z-index 5) are duplicate clips of this same full
    // image (z-index 2) — as they fly apart over their own 600ms
    // animation, they expose the center of this full image behind them.
    // That's correct, it's how a tear should look — the bug was this
    // layer only fading via ripEl's slower 400ms parent-level opacity
    // (packrip, above), so for a chunk of that window the halves had
    // already visibly separated while the complete, uncut image was
    // still sitting there at real opacity underneath — reading as the
    // whole pack art flickering back into view in the gap between them.
    // Fading it out fast and independently, right as the halves start
    // moving, closes that gap: by the time separation is visible, this
    // layer is already gone.
    if(packArtBg){
      packArtBg.style.transition = 'opacity 0.16s ease-out';
      requestAnimationFrame(() => { packArtBg.style.opacity = '0'; });
    }

    // The actual "impact" moment — previously the tear cut straight from
    // flying halves into the reveal screen with nothing bridging them.
    // This fires a beat after the tear itself (letting the halves start
    // flying apart first) so it reads as the pack RELEASING something,
    // not simultaneous with the tear.
    if(!reducedMotion()){
      setTimeout(() => {
        const flash = el('div','tear-flash go'); document.body.appendChild(flash);
        const shock = el('div','tear-shockwave go'); document.body.appendChild(shock);
        ...
        setTimeout(() => { flash.remove(); shock.remove(); }, 650);
      }, 140);
    }
    ...
    setTimeout(()=>{
      ...
      // for the tear screen itself), which is already longer than this
      // 620ms handoff on ANY device, not just slow ones. Left alone,
      // the confetti canvas (z-index 150, above the reveal screen's
      // z-index 100) would keep rendering their tail end on top of the
      // newly-mounted card reveal — reading as the pack flashing back
      // into view right as the new screen appears. Cutting them off
      // here preserves their intended lingering feel for the moment
      // they were actually designed for (the tear itself) without ever

...
const ACHIEVEMENT_COLORS = {
  first_pull: '#22d3ee', pack_rat: '#22d3ee', century_club: '#22d3ee',
  first_trade: '#34d399', dealmaker: '#34d399',
  duelist: '#f87171', win_streak_5: '#f87171',
  dealer: '#fbbf24', high_roller: '#fbbf24',
  set_complete: '#a78bfa', collector: '#a78bfa', whale_watcher: '#a78bfa',
  popular: '#f472b6'
};

/* ============================================================
   BATTLE BADGES — a separate, harder-earned tier from the
   achievement badges above. One per battle win, in order, once
   the battle feature exists (see isBattleWon()/awardBattleBadge()
   call site notes near the battle module for where these get
   granted — not wired yet since there's no battle feature to win
   yet). Deliberately original geometric-gem designs (not modeled
   on any existing gym-badge-style franchise art) that escalate in
   color/facet complexity from badge 1 to badge 8, so the set reads
   as a genuine progression rather than 8 reskins of one shape.
   Reads from `userProfile.battle_badges` (expected: an array of
   earned keys) — that column doesn't exist yet, so this defaults
   safely to "nothing earned" everywhere until it's added. */
const BATTLE_BADGE_META = [
  { key: 'stonewall', label: 'Stonewall', color: '#94a3b8', desc: 'Win your 1st battle' },
  { key: 'riptide',   label: 'Riptide',   color: '#38bdf8', desc: 'Win your 2nd battle' },
  { key: 'ember',     label: 'Ember',     color: '#f97316', desc: 'Win your 3rd battle' },
  { key: 'verdant',   label: 'Verdant',   color: '#22c55e', desc: 'Win your 4th battle' },
  { key: 'voltage',   label: 'Voltage',   color: '#eab308', desc: 'Win your 5th battle' },
  { key: 'glacier',   label: 'Glacier',   color: '#22d3ee', desc: 'Win your 6th battle' },
  { key: 'eclipse',   label: 'Eclipse',   color: '#a78bfa', desc: 'Win your 7th battle' },
  { key: 'prism',     label: 'Prism',     color: '#f472b6', desc: 'Win your 8th battle — completes the set' },
];
...
// Persists a win server-side by asking the DB for "the next unearned
// badge in order" rather than the client naming one — see the SQL this
// depends on (award_next_battle_badge) in the project notes. Known
// limitation: since match resolution itself runs entirely client-side
// (no server-authoritative battle engine yet), this trusts the client's
// claim that a win happened, same as any purely local single-player game
// would. Fine for a cosmetic/vanity badge track; would need real
// server-side match validation before this could back anything with
// stakes attached (ranked ladder, rewards with resale value, etc).
async function awardNextBattleBadge(){
  if (guestMode || !session?.user) return null;
  try {
    const { data, error } = await sb.rpc('award_next_battle_badge');
    if (error) throw error;
    if (profile) profile.battle_badges = data;
    if (document.getElementById('account-section') && profile) renderAccountArea(session.user, profile);
    return data;
  } catch (e) {
    console.warn('award_next_battle_badge RPC unavailable (run the battle-badges SQL in Supabase) — badge not persisted:', e.message);
    return null;
  }
}
...
async function maybePromptMarketingOptIn(){
  ...
  sheet.innerHTML = `
    ...
    <div class="sub">Want emails about new set drops and deals? You can unsubscribe anytime.</div>
  ...
  overlay.addEventListener('click', (e)=>{ if(e.target===overlay) overlay.remove(); });
  $('#optin-no-btn', sheet).addEventListener('click', () => overlay.remove());
  $('#optin-yes-btn', sheet).addEventListener('click', async () => {
    overlay.remove();
    try{
      const { error } = await sb.from('profiles').update({ marketing_opt_in: true }).eq('id', session.user.id);
      if(error) throw error;
      if(profile) profile.marketing_opt_in = true;
      toast('Got it — you\'re on the list!');
    }catch(e){ toast('Could not save that — try again from your account settings later.'); }
  });
}
...
function renderBadgesSection(userProfile) {
  const tiles = [];
  if (userProfile?.founding_member) {
    tiles.push(badgeTileHTML(
      badgeIconSVG(FOUNDING_ICON_PATH, 32),
      `Founding #${userProfile.founding_member_number}`,
...
function renderBattleBadgesSection(userProfile){
  ...
  const tiles = BATTLE_BADGE_META.map(b => {
    ...
    return `
      <div class="battle-badge-tile${has ? ' earned' : ''}" title="${has ? escapeHtml(b.label) : 'Locked — ' + escapeHtml(b.desc)}">
  ...
  return `
    <div class="battle-badges-section">
      ...
      <p class="hint" style="margin:2px 0 10px;">Earned one at a time by winning battles. All 8 unlocks a Champion badge and permanent VIP rank — for life.</p>
...
function renderBattleDuel(deck){
  ...
  async function endDuel(){
    ...
    if (state.winner === 'player' && !awarded) {
      awarded = true;
      const before = new Set(profile?.battle_badges || []);
      const after = await awardNextBattleBadge();
      const newBadge = after && BATTLE_BADGE_META.find(b => after.includes(b.key) && !before.has(b.key));
      const banner = el('div', 'battle-result-banner win');
      banner.innerHTML = `<div>🏆 You won the duel!</div>${newBadge ? `<div style="margin-top:6px; font-size:12.5px;">Earned the <b>${newBadge.label}</b> Battle Badge</div>` : ''}`;
      wrap.insertBefore(banner, $('.battle-arena', wrap));
    } else if (state.winner === 'ai') {
      const banner = el('div', 'battle-result-banner lose');
      banner.innerHTML = `<div>Defeated — try again anytime, your deck is saved.</div>`;
      wrap.insertBefore(banner, $('.battle-arena', wrap));
    }
...
function renderAllBadgesPanel(userProfile){
  ...
  if (userProfile) {
    ...
    rows.push(`
      <div class="tier-ladder-row${hasFounding ? ' current-tier-row' : ''}" style="opacity:${hasFounding ? '1' : '0.5'}; ${hasFounding ? `--tier-color:${FOUNDING_COLOR};` : ''}">
        <span class="icon" style="color:${hasFounding ? FOUNDING_COLOR : 'var(--dim)'};">${badgeIconSVG(FOUNDING_ICON_PATH, 18)}</span>
        <span class="name">Founding Member</span>
        <span class="req">Among the first 1,000 accounts — 2× earned credits, forever</span>
        ${hasFounding ? '<span style="font-size:10px; font-weight:800; color:'+FOUNDING_COLOR+'; flex-shrink:0;">EARNED</span>' : ''}
      </div>`);
  }
  ...
  Object.keys(ACHIEVEMENT_META).forEach(key => {
    ...
    rows.push(`
      <div class="tier-ladder-row${earned ? ' current-tier-row' : ''}" style="opacity:${earned ? '1' : '0.5'}; ${earned ? `--tier-color:${color};` : ''}">
        <span class="icon" style="color:${earned ? color : 'var(--dim)'};">${badgeIconSVG(ACHIEVEMENT_ICON_PATHS[key] || '', 18)}</span>
...
function renderAccountArea(user, userProfile) {
  ...
  if (rankHeaderEl) {
    rankHeaderEl.innerHTML = (user && !isAdmin) ? renderTierProgressBanner(userProfile) : '';
  }
...
function openGetCreditsModal(lowBalance=false){
  ...
  sheet.innerHTML = `
    ...
    <div class="hint" style="margin-bottom:18px;">You both get +${refBonus.toLocaleString()} credits when they sign up.</div>
    ${shareSectionHTML()}
  `;
  overlay.appendChild(sheet); document.body.appendChild(overlay);
  overlay.addEventListener('click', (e)=>{ if(e.target===overlay) overlay.remove(); });

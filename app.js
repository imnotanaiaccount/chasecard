...
// "STARTER DECK -Straw Hat Crew- [ST-01]" — "STARTER DECK" is identical on
// every one, so it's the part of the label LEAST likely to help someone
// tell packs apart there, while eating the space the actually-
// distinguishing text (which box/character) needs. For any name containing
// "Starter Deck", show only what comes after the first hyphen (e.g. "Straw
// Hat Crew- [ST-01]"). Non-starter-deck packs (boosters, etc.) render
// unchanged — there's no repeated boilerplate prefix to strip there.
// setMeta.name itself is untouched everywhere else (pack hero header,
// pack-count text, share text, etc.) where the fuller wording still helps.
function onePieceGridDisplayName(s){
  if (s.series !== 'One Piece' || !s.name) return s.name;
  if (!/starter deck/i.test(s.name)) return s.name;
  const hyphenIdx = s.name.indexOf('-');
  if (hyphenIdx === -1) return s.name;
  const after = s.name.slice(hyphenIdx + 1).trim();
  return after || s.name;
}

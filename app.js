...
async function onLoggedIn(){
  ...
  if (store.get('pending_welcome_bonus', false)) {
    try {
      ...
      await refreshPackTickets();
      toast('Welcome! You got a free Base Set, Base Set 2 & Fossil pack — check the ticket badge on those sets');
    } catch (e) {
...
// images, just the same art the actual pack-opening screens use. Fossil
// and Jungle have no curated photo in OWN_PACK_ART, so those two will
// come from the TCGdex fallback specifically; if that's ever offline,
// the tile just falls back to a plain colored box with the set name
// rather than staying broken/blank.
...
function renderPromoPackStrip(container, setIds){
  ...
  sets.forEach(async s => {
    try {
      ...
      if (wrap) wrap.innerHTML = `<img src="${urls[0]}" alt="${escapeHtml(s.label)} pack" loading="lazy"/>`;
...
function openAuthModal(resumeSetMeta = null){
  ...
  function renderMainView() {
    sheet.innerHTML = `
      ...
      <div class="hint" style="text-align:center; margin:6px 0 14px;">New accounts get a free Base Set, Base Set 2 &amp; Fossil pack on top of starting credits</div>
    ...
    if (promoStrip) renderPromoPackStrip(promoStrip, ['base1', 'base2', 'fossil']);
...
function renderAllBadgesPanel(userProfile){
  ...
  if (userProfile && isAdmin) {
    // Founding Member's entire payoff is a 2x bonus on *earned* credits —
    // meaningless for an account that already has unlimited credits. Showing
    // it as just another dimmed, seemingly-lockable badge (as it does for
    // real accounts below) reads as "you're missing something," when really
    // it just doesn't apply here. Say that plainly instead.
    rows.push(`
      <div class="tier-ladder-row" style="opacity:0.65;">
        <span class="icon" style="color:var(--dim);">${badgeIconSVG(FOUNDING_ICON_PATH, 18)}</span>
        <span class="name">Founding Member</span>
        <span class="req">Not applicable — admin accounts already have unlimited credits</span>
      </div>`);
  } else if (userProfile) {
    const hasFounding = !!userProfile.founding_member;
    rows.push(`
      <div class="tier-ladder-row${hasFounding ? ' current-tier-row' : ''}" style="opacity:${hasFounding ? '1' : '0.65'}; --tier-color:${FOUNDING_COLOR};">
...
function openGetCreditsModal(lowBalance=false){
  ...
  sheet.innerHTML = `
    ...
    <div class="hint" style="margin-bottom:18px;">You both get +${refBonus.toLocaleString()} credits when they sign up — plus a free Base Set, Base Set 2 &amp; Jungle pack ticket each.</div>
    ${shareSectionHTML()}
  `;
  overlay.appendChild(sheet); document.body.appendChild(overlay);
  overlay.addEventListener('click', (e)=>{ if(e.target===overlay) overlay.remove(); });
  const referralPromoStrip = $('#referral-promo-strip', sheet);
  if (referralPromoStrip) renderPromoPackStrip(referralPromoStrip, ['base1', 'base2', 'jungle']);
  sheet.querySelectorAll('.share-btn').forEach(btn=>{
    btn.addEventListener('click', ()=> claimShareBonus(btn.dataset.platform, btn));
  });

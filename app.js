...
const Prewarm = {
  ...
  async _resolvePackArtUrlsUncached(setMeta) {
    ...
    // no actual booster-pack photography for a set. The first version of
    // this fix only excluded it when `own` specifically had art, but
    // ghConfirmed/jpDirect/niche are ALSO real photographed candidates —
    // Jungle has no `own` entry at all, so its two real GitHub-sourced
    // pack photos (own.length === 0, skipRedundantPackshots false) still
    // let the logo ride along as a third, worse "option" right next to
    // two genuinely different real ones. The logo is only ever the
    // fallback when NOTHING real resolved — so build the real candidates
    // first and only add the logo if that list is empty, rather than
    // gating on any one specific source being present.
    const realArtCandidates = [
      ...own,

...
function generatePack(cards){
  ...
  for(let i=0;i<4;i++) place(pickFromExcluding(commons, usedIds), false);
  for(let i=0;i<3;i++) place(pickFromExcluding(uncommons, usedIds), false);
  ...
  for(let i=0;i<2;i++){
    ...
    place(card, card ? classify(card.rarity).id >= 3 : false);
  }

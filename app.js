...
function getTierProgress(userProfile){
  ...
  // Battle Champion (all 8 battle badges) is a COSMETIC badge only — see
  // the Battle Trophies section for where it renders. It must never
  // override the Rank shown/paid out here: this banner's dailyBonus
  // figure is a promise of real credits, and profiles.premium_tier
  // (not battle-badge count) is what actually determines what gets
  // paid server-side. Faking `current` to the VIP tier object here
  // previously told rookie-tier Battle Champions they had a permanent
  // 250,000/day bonus they were not actually receiving. Rank still
  // only comes from packs/referrals, same as everyone else — the
  // battleChampion flag below is for cosmetic badge display only.
...
const OPTCG_SETS_CACHE_KEY = 'cache_punkrecords_sets_v3'; // v2->v3: numeric ids (e.g. "569001") no longer get prepended at all — see formatOnePiecePackName
...
function packCompositionFor(game, packSize){
  if(game === 'yugioh') return { commons: 7, uncommons: 0, hitSlots: 1, ygo: true };
...
function packStructureLabel(game, packSize){
  ...
  if(c.ygo) return `${c.commons} common · 1 guaranteed rare-or-higher · ${c.hitSlots} common-or-foil slot`;
...
function generatePack(cards, game, packSize){
  ...
  const activeWeights = (!ygo && packSize === 10) ? HIT_WEIGHTS_REAL : HIT_WEIGHTS;
  for(let i=0;i<nHits;i++){
    ...
    // no shot at Double Rare and up).
    if(!ygo && packSize === 10 && i > 0){
...
function renderBattleBadgesSection(userProfile){
  ...
  return `
    <div class="battle-badges-section">
      ...
      <p class="hint" style="margin:0 0 10px;">Each trophy needs more wins than the last. All 8 unlocks Champion status and a permanent VIP rank badge on your profile (cosmetic — separate from VIP membership's credit bonuses).</p>
      <div class="battle-badges-grid">${tiles}</div>
      ${champion ? `<div class="battle-champion-banner">${badgeIconSVG(TROPHY_ICON_PATH, 16)}<span>Undefeated — permanent VIP rank badge unlocked</span></div>` : ''}
    </div>`;
}
...
function renderBattleHome(){
  ...
  wrap.innerHTML = `
    <div class="section-title">Battle</div>
    <div class="hint" style="margin:-4px 0 14px;">Original card-duel mode — win battles to earn Battle Trophies. Beat all 8 for Champion status and a permanent VIP rank badge (cosmetic — separate from VIP membership's credit bonuses).</div>

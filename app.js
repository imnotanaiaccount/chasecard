...
/* ============================================================
   Styles — moved to styles.css (v6 LAYER)
   ------------------------------------------------------------
   This used to build a second <style> element at runtime and
   append it to <head> after styles.css had already loaded — a
   silent, always-behind-in-the-cascade duplicate of some of
   styles.css's own rules (.account-badge, .account-header, ...)
   plus a big block of rules that only ever lived here. None of
   it depended on runtime data, so it's static CSS moved into
   styles.css verbatim, single source of truth now. See the
   "v6 LAYER" comment near the end of styles.css.
   ============================================================ */
...
// styles.css reads, so the visual side stays pure CSS.
...
function openRevealScreen(setMeta, pack, bgUrl, openingId){
  ...
  function boot(){
    ...
    function revealCurrent(){
      ...
      SFX.flip();
      // Primes the reveal stage's --tier-glow (see .stage::before in styles.css) so the
      // floor under the card is already tinting toward this tier's color during the charge,
      // not just snapping to it once the flip lands. Visual-only counterpart to the vibrate()
      // calls below — fx.js's own FX.charge() would also buzz, which app.js's per-tier
      // vibrate() patterns already cover, so this uses the no-haptic FX.glow() instead.
      // Optional-chained throughout: fx.js not loading is not a reveal-blocking failure.
      window.FX?.glow?.(tier.id);
